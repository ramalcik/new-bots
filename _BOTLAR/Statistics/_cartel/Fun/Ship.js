...
module.exports = {
   ...
   */
    ...
    onRequest: async function (client, message, args) {
        ...
        person._views()
        if(özel.includes(person.id)) return message.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} Tabi Efendim Ship Atarsın.`}).then(x => {
        ...
        let background = await Canvas.loadImage(client.guilds.cache.get(sistem.SUNUCU.GUILD).bannerURL({dynamic: true, format: "png"}) ? client.guilds.cache.get(sistem.SUNUCU.GUILD).bannerURL({dynamic: true, format: "png"}) + `?size=4096` : "https://cdn.discordapp.com/attachments/1019290688136958028/1023941333716566046/thumb-1920-1146731.jpg");
