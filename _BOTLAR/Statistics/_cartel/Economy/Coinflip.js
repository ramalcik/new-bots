...
message.reply({content: `**Bahis Devam Ediyor!** ${message.guild.emojiGöster("munur_cf")}
...
Belirlenen Miktar: \` ${Miktar.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} ${ayarlar.serverName} Parası \``}).then(msg => {
        setTimeout(async () => {
            ...
            message.react(rnd == 1 ?  message.guild.emojiGöster(emojiler.onay_munur) : message.guild.emojiGöster(emojiler.no_munur)).catch(err => {})
            let cc = Miktar*2;
            msg.edit({content: `**Bahis Bitti!** ${rnd == 1 ? message.guild.emojiGöster("munur_cfwin") : message.guild.emojiGöster("munur_cflose")}
\` ${cc.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} ${ayarlar.serverName} Parası \` için bahis döndürülme durdu${rnd == 1 ? " ve kazandın" : " ve kaybettin"}!

${rnd == 1 ? `:tada: **Tebrikler!** Bu oyunu kazandın!` : `${message.guild.emojiGöster(emojiler.no_munur)} **Kaybettin!** Bu oyunu kazanamadın!`}
