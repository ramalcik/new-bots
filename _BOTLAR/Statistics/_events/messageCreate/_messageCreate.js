...
async function dayStatsUpdate (id, channel) {
    ...
    if(kanal) {
        let ramalcim = kanal.guild.members.cache.get(id)
        if(ramalcim && (kanallar.chatKanalı && kanallar.chatKanalı == kanal.id)) {
            let veri = verileriki.get(id) || 0
            if (veri >= 10) { 
                ramalcim.Leaders("chat", 0.10, {type: "MESSAGE", channel: kanal.id})
    ...
    await Stats.updateOne({ userID: id, guildID: global.sistem.SUNUCU.GUILD }, { $inc: { [`allMessage.${days}.${channel}`]: 1} })
...
async function messageXP(id, message) {
    let msgXp = [1,2,3,4]
    await Stats.updateOne({guildID: sistem.SUNUCU.GUILD, userID: id}, {$inc: {"messageXP": msgXp[Math.floor(Math.random()*msgXp.length)]}}, {upsert: true})
    ...
    if(_getStat) {
      ...
      if (yeniLevel <= _getStat.messageXP) {
        ...
        await Stats.updateOne({guildID: sistem.SUNUCU.GUILD, userID: id}, {$inc: {"messageLevel": 1, "messageXP": 0,}}, {upsert: true})
        ...
        if ((_getStat.messageLevel).toString().length == 1) {
            ...
            message.reply({content: `**Tebrikler!** ${message.member.user.tag}
