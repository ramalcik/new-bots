    ...
    run: async (client, int, args) => {
        ...
        if(author_data && member_data && author_data.FollowUp.includes(member.id) && member_data.Follower.includes(author.id)) {
            ...
            if(member_data && member_data.FollowUp.includes(author.id)) {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.onay_munur)} **${author.user.tag}** ile birbirinizi **<t:${String(Date.now()).slice(0,10)}:R>** karşılıklı takip ederek arkadaş oldunuz.`)
                .catch(err => {
                ...
                member.send(`${int.guild.emojiGöster(emojiler.no_munur)} **${author.user.tag}** sizi **<t:${String(Date.now()).slice(0,10)}:R>** takip etmeye başladı.`)
                .catch(err => {
