...
module.exports = {
    Isim: "isim",
    Komut: ["i","nick"],
    Kullanim: "isim <@munur/ID> <İsim/Nick>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(message.author.id === ramalcim.id) return message.reply(cevaplar.kendi).then(x => setTimeout(() => {x.delete()}, 7500))
    if(ramalcim.user.bot) return message.reply(cevaplar.bot).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    if(!ramalcim.manageable) return message.reply(cevaplar.dokunulmaz).then(x => setTimeout(() => {x.delete()}, 7500))
    if(message.member.roles.highest.position <= ramalcim.roles.highest.position) return message.reply(cevaplar.yetkiust).then(x => setTimeout(() => {x.delete()}, 7500))
    ...
    ramalcim.setNickname(`${ayarlar.type ? ramalcim.user.username.includes(ayarlar.tag) ? ayarlar.tag + " " : (ayarlar.tagsiz ? ayarlar.tagsiz + " " : (ayarlar.tag || "")) : ``}${setName}`).catch(err => message.channel.send(cevaplar.isimapi));
    ...
    ramalcim.Rename(`${setName}`, message.member, "İsim Güncelleme")
    let isimLog = message.guild.kanalBul("isim-log")
    if(isimLog) isimLog.send({embeds: [new cartelinEmbedi().setDescription(`${ramalcim} isimli üyenin ismi ${message.member} tarafından \`${tarihsel(Date.now())}\` tarihinde "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak güncellendi.`)]})
    message.reply({embeds: [new cartelinEmbedi().setDescription(`${ramalcim} üyesinin ismi "${ayarlar.isimyas ? `${isim} | ${yas}` : `${isim}`}" olarak değiştirildi${isimveri.Names ? isimveri.Names.length > 0 ? 
        `, bu üye daha önce bu isimlerle kayıt olmuş.\n\n${ramalcim} üyesinin toplamda **${isimveri.Names.length}** isim kayıtı bulundu.
__Aşağıda son 10 işlem listelenmekte__

${isimler}\n\nÖnceki isimlerine \`${sistem.botSettings.Prefixs[0]}isimler <@munur/ID>\` komutuyla bakarak kayıt işlemini\n gerçekleştirmeniz önerilir.`
: "." : "."}`)]}).then(msg => {
