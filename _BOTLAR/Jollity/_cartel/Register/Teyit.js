...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!roller.teyitciRolleri.some(oku => message.member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has('ADMINISTRATOR')) return message.reply(cevaplar.yetersiz).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    let teyit = await Kullanici.findOne({ _id: ramalcim.id }) || [];
    let teyitBilgisi;
    if(teyit.Records){
      ...
      let taglılar = teyit.Records.filter(x => {
        let ramalcim = message.guild.members.cache.get(x.User)
        return ramalcim && ramalcim.user.username.includes(ayarlar.tag)
      }).length

      teyitBilgisi = `${ramalcim} toplam **${erkekTeyit+kizTeyit}** kişi kayıt etmiş! (**${erkekTeyit}** erkek, **${kizTeyit}** kadın, ${ayarlar.type ? `**${taglılar}** taglı, ` : ``}**${çıkanlar}** çıkanlar)
${erkekTeyit+kizTeyit > 0 ? `Kayıt edilen bazı kişiler: ${bazıları ? bazıları.length > 10  ? `${bazıları.slice(0, 10).join(", ")} ve ${erkekTeyit + kizTeyit - 10} daha fazla ` : bazıları.slice(0, 10).join(",") : bazıları }`  : ''}`;
      ...
      teyitBilgisi = `${ramalcim} isimli üyenin teyit bilgisi bulunamadı.`
    }
    message.reply({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(teyitBilgisi)]});
    }
};