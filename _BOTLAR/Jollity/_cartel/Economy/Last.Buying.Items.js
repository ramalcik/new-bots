...
module.exports = {
 ...
 .setStyle('DANGER');
    ...
    Users.findOne({_id: ramalcim.id }, async (err, res) => {
      if (!res) return message.channel.send({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli üyenin satın alımı bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      if(!res.Inventory) return message.channel.send({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli üyenin satın alımı bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      ...
      const curPage = await message.channel.send({
        embeds: [embed.setDescription(`${ramalcim}, üyesin satın alımı bilgisi yükleniyor... Lütfen bekleyin...`)],
        components: [row], fetchReply: true,
      }).catch(err => {});
    
      await curPage.edit({embeds: [embed.setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Inventory.length || 0}\` adet satın alımı mevcut.
...
${pages[currentPage - 1].map((value, index) => `**${value.Name}** isimli ürünü **${value.Coin > 0 ? value.Coin.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,") + ` ${ayarlar.serverName} Parası${value.Gold > 0? " ve" : ""}` : ``} ${value.Gold > 0 ? value.Gold + ` Altın` : ``}** fiyatına satın aldı.`).join("\n")}`)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Inventory.length || 0}\` adet satın alımı mevcut.
...
${pages[currentPage - 1].map((value, index) => `**${value.Name}** isimli ürünü **${value.Coin > 0 ? value.Coin + ` ${ayarlar.serverName} Parası${value.Gold > 0? " ve" : ""}` : ``} ${value.Gold > 0 ? value.Gold + ` Altın` : ``}** fiyatına satın aldı.`).join("\n")}`)]
      ...
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Inventory.length || 0}\` adet satın alımı mevcut.`)],
          components: [],
        }).catch(err => {});
      })
    })
