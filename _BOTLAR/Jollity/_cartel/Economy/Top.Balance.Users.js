...
${Zenginler.sort((a,b) => {
    ...
    }).filter(x => message.guild.members.cache.get(x._id) && !sistem._rooter.rooters.includes(x._id) && !message.guild.members.cache.get(x._id).user.bot).slice(0, 30).map((x, index) => `\` ${index+1} \` ${x._id ? message.guild.members.cache.get(x._id) : `<@${x._id}>`} \` ${x.Coin.toFixed(0).replace(/(\d)(?=(\d\d\d)+(?!\d))/g, "$1,")} \` ${message.guild.emojiGöster(emojiler.Görev.Para)} | \` ${x.Gold} \` ${message.guild.emojiGöster(emojiler.Görev.Altın)} ${x._id == message.member.id ? `**(Siz)**` : ``}`).join("\n")}`)]}).then(x => {
      setTimeout(() => {
        x.delete().catch(err => {})
      }, 20000);
    })
