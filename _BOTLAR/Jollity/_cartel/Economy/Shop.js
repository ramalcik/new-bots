const { Client, Message, MessageEmbed, MessageButton, MessageActionRow, MessageAttachment, MessageSelectMenu } = require("discord.js");
let Shops = require('../../../../_SYSTEM/Databases/Schemas/Others/Economy.Shop.Items');
...
let ürünlistesiii = new MessageActionRow().addComponents(
...
.addOptions(
    arrayItems.length > 0 ? arrayItems : {label: "Kapalı Mağaza", description: "Mağazamız kapalıdır!", value: "1233223423"}
        ...
        if(sistem._rooter.rooters.includes(message.member.id) || roller.kurucuRolleri.some(x => { message.member.roles.cache.has(x)})) {
            if(args[0] == "sil") {
        ...
        collector.on('collect', async i => { 
            if(i.customId == "ürüncükal") { 
                ...
                if(!ürünBilgisi) return i.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} **Başarısız!** böyle bir ürün bulunamadı, lütfen ürünü seçin!`, ephemeral: true});
                ...
                let ramalcim = message.guild.members.cache.get(i.user.id)
                if(coin > 0 && para < coin) return i.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} **Başarısız!** Gereken ${coin} ${ayarlar.serverName} Parası bulunamadığından satın alamazsın!`, ephemeral: true});
                if(gold > 0 && altın < gold) return i.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} **Başarısız!** Gereken ${gold} Altın bulunamadığından satın alamazsın!`, ephemeral: true});
                let dattaaa = await Users.findOne({_id: ramalcim.id})
                if(dattaaa && dattaaa.Inventory) {
                    let ürünlerrrrr = []
                    dattaaa.Inventory.forEach(x => {
                        ürünlerrrrr.push(x.Name)
                    })
                    if(ürünlerrrrr.includes(isim)) return msg.delete().catch(err => {}),i.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} **Başarısız!** \`${isim}\` isimli ürün envanterinde bulunduğundan dolayı tekrardan satın alamazsın.`, ephemeral: true}),message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined).catch(err => {})
                }
                if(coin > 0) {
                    await client.Economy.updateBalance(ramalcim.id, coin, "remove", 1)
                } 
                if(gold > 0) {
                    await client.Economy.updateBalance(ramalcim.id, gold, "remove", 0)
                }
                if(ürünBilgisi.role) {
                    ramalcim.roles.add(ürünBilgisi.roleID).catch(err => {})
                } 
                let alınanÜrün = {
                    Name: isim,
                    Coin: coin,
                ...
                if(tip == "badge") Users.updateOne({_id: ramalcim.id}, {$push: { "Badges": {Name: isim, Emoji: emoji}}}, {upsert: true})
                await Users.updateOne({_id: ramalcim.id}, {$push: { "Inventory": alınanÜrün}}, {upsert: true})
                msg.delete().catch(err => {})
                ...
                .setFooter("alınan ürün envanter bilgisine girildi ve kayıtlara tutuldu.")
                ]})
                i.reply({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla **${isim}** isimli ürünü <t:${String(Date.now()).slice(0, 10)}:R> satın aldınız.`).setFooter("ürün hakkında sorular için sunucu sahibine başvurun.")], ephemeral: true})
                message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {})
