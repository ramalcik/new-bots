...
module.exports = {
   ...
   */
    ...
    onRequest: async function (client, message, args) {
        if(sistem._rooter.rooters.includes(message.member.id) && args[0] == "forcedelete") {
            let ramalcim = args[1]
            if(!ramalcim) return;
            await Users.deleteOne({_id: ramalcim})
        ...
        let kullanici = message.mentions.users.first() || client.guilds.cache.get(sistem.SUNUCU.GUILD).members.cache.get(args[0]) || message.member;
        if(!kullanici) return message.reply(cevaplar.üye).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
        let ramalcim = client.guilds.cache.get(sistem.SUNUCU.GUILD).members.cache.get(kullanici.id)
        let para = await client.Economy.viewBalance(ramalcim.id, 1)
        let altın = await client.Economy.viewBalance(ramalcim.id, 0)
        // Canvas
let canvas = Canvas.createCanvas(1080, 400),
    ...
    let background = await Canvas.loadImage(client.guilds.cache.get(sistem.SUNUCU.GUILD).banner ? client.guilds.cache.get(sistem.SUNUCU.GUILD).banner ? client.guilds.cache.get(sistem.SUNUCU.GUILD).bannerURL({dynamic: true, format: "png"}) + `?size=4096` : "https://cdn.discordapp.com/attachments/946751524028162118/947678638315298866/open-the-door-android-iphone-desktop-hd-backgrounds-wallpapers-1080p-4khd-wallpapers-desktop-background-android-iphone-1080p-4k-pqrfj.png" : "https://cdn.discordapp.com/attachments/946751524028162118/947678638315298866/open-the-door-android-iphone-desktop-hd-backgrounds-wallpapers-1080p-4khd-wallpapers-desktop-background-android-iphone-1080p-4k-pqrfj.png");
    ...
    ctx.stroke();
      ...
      ctx.font = applyText(canvas, ramalcim.user.tag + " kullanıcısının HESABI", 40, 600, "Bold");
      ctx.fillText(ramalcim.user.tag + " kullanıcısının HESABI", canvas.width - 740, canvas.height - 230);
    ...
    ctx.closePath();
    ctx.clip();
    const avatar = await Canvas.loadImage(ramalcim.user.avatar ? ramalcim.user.avatarURL({dynamic: true , format: "png"}) : "https://cdn.discordapp.com/attachments/915702481776963615/928900758135509022/dd83hdp-ef7fc5cd-ac92-4ad1-adfc-8e2e933ce256.png");
    ctx.drawImage(avatar, 58, 70, 270, 270);
        ...
        let Hesap = await Users.findById({_id: ramalcim.id}) 
        ...
        const attachment = new MessageAttachment(canvas.toBuffer(), 'munur_economy.png');

        message.reply({content: `💳 | ${ramalcim.id == message.member.id ? `${kullanici}` : `${kullanici} üyesinin`} **${ayarlar.serverName} Parası** ve **Altın** ${ramalcim.id == message.member.id ? "hesabın" : "hesabı"} aşağıda görüntülenmektedir.`, files: [attachment], components: message.member.id == ramalcim.id ? [ekonomiPanel] : null}).then(async (x) => {
...
ctx.fillStyle = "#ffffff";
ctx.fillRect(0, 0, 1080, 400);
let background = await Canvas.loadImage(client.guilds.cache.get(sistem.SUNUCU.GUILD).banner ? client.guilds.cache.get(sistem.SUNUCU.GUILD).bannerURL({dynamic: true, format: "png"}) + `?size=4096` : "https://cdn.discordapp.com/attachments/946751524028162118/947678638315298866/open-the-door-android-iphone-desktop-hd-backgrounds-wallpapers-1080p-4khd-wallpapers-desktop-background-android-iphone-1080p-4k-pqrfj.png");
ctx.drawImage(background, 0, 0, 1080, 400);
ctx.restore();
ctx.closePath();
...
ctx.fillText(kod, 525, 350 );
    ...
    collector.on('collect',async (m) => {
        m.react(m.guild.emojiGöster(emojiler.onay_munur))
        let ramalcim = m.guild.members.cache.get(m.member.id)
        kanal.send({content: `**Tebrikler!** Hızlı Ol Ve Kazan etkinliğini ${ramalcim} kazandı! ${m.guild.emojiGöster("a_rich")}` , files: [], files: []}).then(x => setTimeout(() => {
            x.delete()
        }, 10000))
msg.delete().catch(err =>{})
        if(ramalcim) return await client.Economy.updateBalance(ramalcim.id, Number(odül), "add", 1)
    });
...
ctx.fillText("SADECE BIR KEZ HAKKIN VAR!", 190, 310 + 40 );
    ...
    collector.on('collect', async i => {
        if (i.customId === String(cevap)) {
            if(basanlar.includes(i.user.id)) return await i.reply({content: `${i.guild.emojiGöster(emojiler.no_munur)} Cevap hakkınızı doldurmuşsunuz. Üzgünüm!`, ephemeral: true});
            ...
            let ramalcim = i.guild.members.cache.get(i.user.id)
	                       kanal.send({content: `**Tebrikler!** Hızlı Ol Ve Kazan etkinliğini ${ramalcim} kazandı! ${i.guild.emojiGöster("a_rich")}` , files: []}).then(x => setTimeout(() => {
                            x.delete()
                        }, 10000))
msg.delete().catch(err =>{})
            basanlar = []
        ...
        if(i.customId != String(cevap)) {
            ...
            await i.reply({ content: `${i.guild.emojiGöster(emojiler.no_munur)} **Hay Aksi!** Yanlış, artık birdahaki sorulara. Cevap hakkınız doldu!`, ephemeral: true})
        }
    });
...
ctx.fillText("SADECE BIR KEZ HAKKIN VAR!", 260, 310 + 40 );
    ...
    collector.on('collect', async i => {
        if (i.customId === String(cevap)) {
            ...
            let ramalcim = i.guild.members.cache.get(i.user.id)
            basanlar = []
                    kanal.send({content: `**Tebrikler!** Cevabı bul etkinliğini ${ramalcim} kazandı! ${i.guild.emojiGöster("a_rich")}` , files: []}).then(x => setTimeout(() => {
                        x.delete()
                    }, 10000))
msg.delete().catch(err =>{})
            if(ramalcim) return await client.Economy.updateBalance(ramalcim.id, Number(odül), "add", 1)
...
ctx.fillText("SADECE BIR KEZ HAKKIN VAR!", 260, 310 + 40 );
    ...
    collector.on('collect', async i => {
        if (i.customId === String(cevap)) {
            ...
            let ramalcim = i.guild.members.cache.get(i.user.id)
                    kanal.send({content: `**Tebrikler!** Doğru kasayı bul etkinliğini ${ramalcim} kazandı! ${i.guild.emojiGöster("a_rich")}` , files: []}).then(x => setTimeout(() => {
                        x.delete()
                    }, 10000))
msg.delete().catch(err =>{})
            basanlar = []
        ...
        if(i.customId != String(cevap)) {
            if(basanlar.includes(i.user.id)) return  await i.reply({content: `${i.guild.emojiGöster(emojiler.no_munur)} Cevap hakkınızı doldurmuşsunuz. Üzgünüm!`, ephemeral: true});
            basanlar.push(i.user.id)
            await  await i.reply({ content: `${i.guild.emojiGöster(emojiler.no_munur)} **Hay Aksi!** Yanlış, artık birdahaki sorulara. Cevap hakkınız doldu!`, ephemeral: true})
        }
    });
