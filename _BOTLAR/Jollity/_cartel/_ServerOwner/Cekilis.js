...
module.exports = {
  ...
  onLoad: function (client) {
    client.on('modalSubmit', async (modal) => {
        ...
        if(modal.customId == "cekilisOlustur") {
            ...
            await modal.deferReply({ ephemeral: true })
            return await modal.followUp({content: `Başarıyla ${kanal} kanalında çekiliş başlatıldı. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
        }
        if(modal.customId == "cekilişBitir") {
            client.giveawaysManager.end(modal.getTextInputValue("id"))
            await modal.deferReply({ ephemeral: true })
            return await modal.followUp({content: `Başarıyla çekiliş bitirildi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
        }
        if(modal.customId == "geRoll") {
            client.giveawaysManager.reroll(modal.getTextInputValue("id"))
            .then(async (x) => {
                await modal.deferReply({ ephemeral: true })
                return await modal.followUp({content: `Başarıyla çekiliş tekrarlandı. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
            })
            .catch(async (err) => {
                await modal.deferReply({ ephemeral: true })
                return await modal.followUp({content: `Tekrardan kazanan belirlenmesi için çekilişin bitmesi gerek. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
            })
        }
    })
  },
  ...
  onRequest: async function (client, message, args) {
    if((roller.çekilişHakkı && !roller.çekilişHakkı.some(x => message.member.roles.cache.has(x))) && !message.member.permissions.has("ADMINISTRATOR") && !roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)))  return message.reply({content: `${cevaplar.yetersiz}`}).then(x => {
    ...
    let Row = new MessageActionRow().addComponents(
        ...
        .setStyle("SUCCESS")
        .setEmoji(message.guild.emojiGöster("munur_giveaway")),
        new MessageButton()
    ...
    await load.edit({
        components: [Row],
        content: `${message.guild.emojiGöster("munur_giveaway")} ${message.guild.emojiGöster("munur_giveaway")} **ÇEKİLİŞ** ${message.guild.emojiGöster("munur_giveaway")} ${message.guild.emojiGöster("munur_giveaway")}`,
        embeds: [
            new cartelinEmbedi().setColor("RANDOM")
