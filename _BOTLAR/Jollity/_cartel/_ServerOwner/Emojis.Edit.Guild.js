...
module.exports = {
  ...
  onRequest: async function (client, msg, args) {
    if(!sistem._rooter.rooters.includes(msg.member.id) && !msg.member.permissions.has('ADMINISTRATOR') && !roller.kurucuRolleri.some(oku => msg.member.roles.cache.has(oku))) return message.reply(cevaplar.yetersiz).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
...
function EmojiYükle(link, ad, message) {
    ...
    if(emojiFind) {
        ...
        message.reply(`Başarıyla ${emojiFind} emojisinin ismi **${ad}** olarak güncellendi. ${message.guild.emojiGöster(emojiler.onay_munur)}`).then(x => {
            setTimeout(() => {
                x.delete().catch(err => {})
                message.reply(message.guild.emojiGöster(emojiler.onay_munur))
            }, 7500);
        })
    } else {
        message.reply(`Belirtilen isimde rol bulunamadı. ${cevaplar.prefix}`).then(x => {
            setTimeout(() => {
                x.delete().catch(err => {})
                message.reply(message.guild.emojiGöster(emojiler.no_munur))
            }, 7500);
        })
    }
}