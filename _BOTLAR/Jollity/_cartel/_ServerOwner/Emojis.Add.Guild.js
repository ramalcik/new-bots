...
module.exports = {
  ...
  onRequest: async function (client, msg, args) {
    ...
    if(!sistem._rooter.rooters.includes(msg.member.id) && !msg.member.permissions.has('ADMINISTRATOR') && !roller.üstYönetimRolleri.some(oku => msg.member.roles.cache.has(oku)) && !roller.kurucuRolleri.some(oku => msg.member.roles.cache.has(oku))) return message.reply(cevaplar.yetersiz).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
...
function EmojiYükle(link, ad, message) {
  ...
  .then(emoji => message.channel.send({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla \`${emoji.name}\` adında emoji oluşturuldu. (${emoji})`)]}).then(x => {
