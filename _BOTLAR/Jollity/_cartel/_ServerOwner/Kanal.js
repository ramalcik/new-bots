...
module.exports = {
  ...
  onLoad: async function (client) {
    client.on('modalSubmit', async (modal) => {
        ...
        if(modal.customId == "kanal_ismi") {
            ...
            await modal.deferReply({ ephemeral: true })
            await modal.followUp({content: `Başarıyla **${Select.name}** kanalının ismi **${isim}** olarak değiştirildi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
            let logKanalı = guild.kanalBul("guild-log")
            if(logKanalı) logKanalı.send({embeds: [new cartelinEmbedi()
            .setFooter(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true}))
            .setThumbnail(guild.iconURL({dynamic: true}))
            .setDescription(`**${guild.name}** sunucusuna ait ${Select ? Select : "#deleted-channel"} kanalı ${ramalcim} tarafından "${Select ? Select.name : isim}" olan ismi "${isim}" olarak <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`)
            ]})
        ...
        if(modal.customId == "kanal_limit") {
            ...
            await modal.deferReply({ ephemeral: true })
            await modal.followUp({content: `Başarıyla ${Select ? Select : "#deleted-channel"} kanalının limiti "${isim}" olarak değiştirildi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
            let logKanalı = guild.kanalBul("guild-log")
            if(logKanalı) logKanalı.send({embeds: [new cartelinEmbedi()
            .setFooter(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true}))
            .setThumbnail(guild.iconURL({dynamic: true}))
            .setDescription(`**${guild.name}** sunucusuna ait ${Select ? Select : "#deleted-channel"} kanalı ${ramalcim} tarafından limiti **${isim}** olarak <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`)
            ]})
  ...
  onRequest: async function (client, message, args) {
    if(!roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has('ADMINISTRATOR')) return message.reply(cevaplar.yetersiz),message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined).then(x => setTimeout(() => {
    ...
    let Row_Two = new MessageActionRow()
            .addComponents(
                ...
                new MessageButton()
                .setCustomId("_iptal")
                .setEmoji(message.guild.emojiGöster(emojiler.no_munur).id)
                .setStyle("SECONDARY"),
            )


                let embed = new cartelinEmbedi().setThumbnail(message.guild.iconURL({dynamic: true}))
...
Kanalın sırasını güncellemek için "🔼 Yukarı" ve "🔽 Aşağı" simgesini kullanarak kanalı taşıyabilirsiniz. Bu sizler için bi kolaylıktır.`)
                ...
                collector.on('collect', async (i) => {
                    ...
                    if(i.customId == "_iptal") {
                        load.delete().catch(err => {})
                        message.reply(message.guild.emojiGöster(emojiler.onay_munur)).catch(err => {})
