...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let notActive = new MessageActionRow().addComponents(
        new MessageButton()
        .setCustomId("aktif")
    ...
    let Active = new MessageActionRow().addComponents(
        new MessageButton()
        .setCustomId("bitir")
 ...
 Etkinlik Kategorileri: \`${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map(x => message.guild.channels.cache.get(x).name).join(", ") : " Ayarlanmamış! "}\``)], components: [etkinlikDurumu ? Active : notActive]}).then(async (msg) => {
        ...
        collector.on('collect', async (i) => {
            if(i.customId == "aktif") {
                ...
                i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla <t:${String(Date.now()).slice(0, 10)}:R> ${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map(x => message.guild.channels.cache.get(x).name).join(", ") + " kategorilerin de": ``} etkinlik **${etkinlikSaniyelikPuan}** saniyelik puan ile başlatıldı.`, ephemeral: true})
                ...
                msg.delete().catch(err => {})
                i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla <t:${String(Date.now()).slice(0, 10)}:R>${etkinlikKanalları.length >= 1 ? etkinlikKanalları.map(x => message.guild.channels.cache.get(x).name).join(", ") + " kategorilerinde ki": ``} etkinlik sonlandırıldı.`, ephemeral: true})
            }
        })
    })
