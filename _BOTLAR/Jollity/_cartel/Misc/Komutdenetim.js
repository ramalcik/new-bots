...
module.exports = {
  ...
  Kullanim: "komutdenetim @munur/ID",
 ...
 .setStyle('DANGER');
    ...
    Users.findOne({_id: ramalcim.id }, async (err, res) => {
      ...
      const curPage = await message.channel.send({
        embeds: [embed.setDescription(`${ramalcim}, üyesin komut bilgileri yükleniyor... Lütfen bekleyin...`)],
        components: [row], fetchReply: true,
      }).catch(err => {});
    
      await curPage.edit({embeds: [embed.setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.
...
${pages[currentPage - 1].map((value, index) => `\` ${index + 1} \` ${global.sistem.botSettings.Prefixs[0] + value.Komut} ${value.Kanal ? message.guild.channels.cache.get(value.Kanal) ? `(${message.guild.channels.cache.get(value.Kanal)})` : `` : ``} <t:${String(value.Tarih).slice(0, 10)}:R>`).join("\n")} `)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.
...
${pages[currentPage - 1].map((value, index) => `\` ${index + 1} \` ${global.sistem.botSettings.Prefixs[0] + value.Komut} ${value.Kanal ? message.guild.channels.cache.get(value.Kanal) ? `(${message.guild.channels.cache.get(value.Kanal)})` : `` : ``} <t:${String(value.Tarih).slice(0, 10)}:R>`).join("\n")}`)]
      ...
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.CommandsLogs.length || 0}\` adet komut kullanımı mevcut.`)],
          components: [],
        }).catch(err => {});
      })
    })
