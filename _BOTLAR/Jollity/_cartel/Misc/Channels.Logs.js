...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    _data = joined.filter(x => message.guild.members.cache.get(x.id)).slice(0, 15).sort((a, b) => b.date - a.date).map((value, index) => {
      ...
      return `\` ${index + 1} \` ${member} ${member.user.tag} (<t:${String(value.date).slice(0, 10)}:R>)`
    }).join("\n")
    ...
    await load.edit({content: null, embeds: [
      new cartelinEmbedi().setDescription(`Aşağıda ${kanal} kanalına son giriş yapan 15 üye listelenmektedir. Toplam da bu kanala giriş yapan ${joined.length} üye bulundu.
