...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let üyeler = message.guild.members.cache.filter(ramalcim => !ramalcim.user.bot && ramalcim.roles.highest.position >= roles.position && !ramalcim.voice.channel)
    var filter = m => m.author.id === message.author.id && m.author.id !== client.user.id && !m.author.bot;
    if(üyeler.size == 0) return message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined);
