...
module.exports = {
  ...
  onRequest: async (client, message, args) => {
      ...
      if(ayarlar.type && ayarlar.isimyas) {
        if(roller.Yetkiler.some(x => message.member.roles.cache.has(x)) && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has('ADMINISTRATOR')) {
          ...
          if(Nickname && message.member.manageable) {
            ...
            return message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
          }
        }
      }
      ...
      }).catch(munur =>  message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined))
    } else {
