...
module.exports = {
  ...
  let pub = message.guild.channels.cache.filter(x => x.parentId == kanallar.publicKategorisi && x.type == "GUILD_VOICE").map(u => u.members.size).reduce((a, b) => a + b)
 ...
 let kontrol;
   if(args[0] == "tüm") {
      kontrol = `\n${message.guild.emojiGöster(emojiler.serverTag)} Public kanallarında **${global.sayılıEmoji(pub || 0)}** üye bulunmakta.
${ayarlar.type ? `${message.guild.emojiGöster(emojiler.serverTag)} Ses kanallarında tagsız **${global.sayılıEmoji(tagsız)}** üye bulunmakta.
${message.guild.emojiGöster(emojiler.serverTag)} Ses kanallarında taglı **${global.sayılıEmoji(taglı)}** üye bulunmakta.\n` : ``}${message.guild.emojiGöster(emojiler.serverTag)} Ses kanallarında yetkili **${global.sayılıEmoji(yetkili)}** üye bulunmakta.`
   } else {
       kontrol = ``;
   }
  message.channel.send(`${message.guild.emojiGöster(emojiler.serverTag)} Ses kanallarında **${global.sayılıEmoji(ses)}** üye bulunmakta.${kontrol}`).then(x =>{
    setTimeout(() => {
      x.delete()
    }, 15000);
  }); 
