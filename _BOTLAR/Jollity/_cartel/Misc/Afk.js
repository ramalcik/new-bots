...
const Afks = require('../../../../_SYSTEM/Databases/Schemas/Others/Users.Afks');
module.exports = {
    Isim: "afk",
    Komut: ["afk"],
    Kullanim: "afk <Sebep>",
	  ...
	  if(GetAfk) {
		...
		message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
          ...
          return message.reply({embeds: [
            new MessageEmbed().setColor("CYAN").setAuthor({name: victim.tag, iconURL: victim.displayAvatarURL()}).setDescription(`${victim} kullanıcısı \`${victimData.sebep ? `${victimData.sebep}\` sebebiyle ` : ""} ${tarih} AFK moduna geçti!`)
          ]}).then(x => {
            setTimeout(() => {
                x.delete()
            }, 7500);
  ...
  onRequest: async function (client, message, args) {
    let GetAfk = await Afks.findById(message.member.id);
    if(GetAfk) return message.reply(`${message.guild.emojiGöster(emojiler.no_munur)} AFK durumdayken tekrardan AFK olamazsın ${message.member}!`).then(x => {
        setTimeout(() => {
            x.delete()
        }, 5000);
    })
