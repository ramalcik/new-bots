...
module.exports = {
  ...
  onRequest: async function (client, message, args, guild) {
    ...
    if (message.channel.permissionsFor(everyone).has('SEND_MESSAGES')) {
      ...
      await message.channel.send({ embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.no_munur)} \`#${message.channel.name}\`  isimli metin kanalına yazmanız kapatıldı.`)] })
    } else {
      await message.channel.permissionOverwrites.edit(everyone.id, { SEND_MESSAGES: true });
      await message.channel.send({ embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} \`#${message.channel.name}\`  isimli metin kanalına yazmanız açıldı.`)] });
    };
