...
module.exports = {
    ...
    Kullanim: "yükseltimler <@munur/ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if (!ramalcim) return message.reply(cevaplar.üye).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
          ...
          Users.findOne({_id: ramalcim.id }, async (err, res) => {
            let msg = await message.channel.send({embeds: [new cartelinEmbedi().setDescription(`Yetkili geçmişi taranıyor...`)]})
          if (!res) return msg.edit({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete().catch(err => {})}, 7500))
          if(!res.StaffLogs) return msg.edit({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli yetkilinin yükseltim geçmiş bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete().catch(err => {})}, 7500))
