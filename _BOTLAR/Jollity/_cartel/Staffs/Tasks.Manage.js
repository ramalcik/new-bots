      ...
      if(modal.customId == "myModal") {
        ...
        if(bilgi && bilgi.rol && bilgi.periyod && bilgi.ödül) {
            ...
            let Row = new MessageActionRow().addComponents(
              ...
              new MessageButton()
                .setStyle("DANGER")
                .setEmoji(guild.emojiGöster(emojiler.no_munur))
                .setCustomId("iptalEt")
                .setLabel("Görev Kurulumunu Sonlandır")
            )
            await modal.reply({components: [Row], embeds: [new cartelinEmbedi().setDescription(`**Merhaba!** ${ramalcim.user.tag}
Şimdi sırada ${rol} görevine gereksinim eklemede. Ekledikten sonra senin için görevleri dağıtacağım. Cevap vermen için "**30 Saniye**" süren var.`)], ephemeral: true })
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "gereksinimEkleme") {
                ...
                await modal.editReply({components: [], embeds: [new cartelinEmbedi().setDescription(`Görev gerekesinim menüsü açıldı işleme burdan devam edilecek. ${i.guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
              }
  
              if(i.customId == "iptalEt") {
                await i.deferUpdate().catch(err => {})
                await modal.editReply({components: [], embeds: [new cartelinEmbedi().setDescription(`Görev oluşturma/güncelleme işlemi başarıyla sonlandırıldı. ${i.guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
              }
  
            })
            collector.on('end', async (collected, reason) => {
              if(reason == "time") {
                await modal.editReply({components: [], embeds: [new cartelinEmbedi().setDescription(`İşleminiz zaman aşımı nedeniyle sonlandırıldı. Daha sonra tekrar deneyin!`)], ephemeral: true})
              }
            })
      ...
      if(modal.customId == "munurKe") {
        ...
        await modal.reply({components: [Row], embeds: [new cartelinEmbedi().setDescription(`**Rol Adı**: ${rol.name} (${rol})
...
Şimdi sırada ${rol} görevine eklediğin gereksinimleri onaylaman gerek. Onaylaman için "**30 Saniye**" süren var.`)], ephemeral: true })
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "görevEkle") {
                await modal.editReply({components: [], embeds: [new cartelinEmbedi().setDescription(`Başarıyla ${rol} görevi verildi/güncelledi ve dağıtım ${rol.members.size} kişi üzerine gerçekleşti. ${i.guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
              ...
              if(i.customId == "iptalEt") {
                await i.deferUpdate().catch(err => {})
                await modal.editReply({components: [], embeds: [new cartelinEmbedi().setDescription(`Görev oluşturma/güncelleme işlemi başarıyla iptal edildi. ${i.guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
              }
      ...
      if(modal.customId == "deleteTasks") {
        ...
        if(guild.roles.cache.get(getInput)) {
            ...
            if(logKanal) logKanal.send({embeds: [new cartelinEmbedi().setDescription(`${belirle} rolününe ait olan ${ramalcimler.map(x => x).slice(0,2).join(", ")} ${ramalcimler.size > 2 ? `ve ${ramalcimler.size - 2} daha fazlası...` : ''} üyeler(üyelerin) <t:${String(Date.now()).slice(0, 10)}:R> ${ramalcim} tarafından tüm görev bilgileri temizlendi.`)]})
            await modal.reply({ephemeral: true, embeds: [new cartelinEmbedi().setDescription(`${ramalcimler.map(x => x.user.tag).slice(0,2).join(", ")} ${ramalcimler.size > 2 ? `ve ${ramalcimler.size - 2} daha fazlası...` : ''} üyeler(üyelerin) \`${belirle.name}\` rolüne ait görev bilgileri temizleniyor...`)]})
            setTimeout(async () => {
                await Tasks.deleteOne({roleID: belirle.id})
                await modal.editReply({embeds: [new cartelinEmbedi().setDescription(`Başarıyla ${belirle} rolüne ait tüm görev verileri temizlendi ve kaldırıldı. ${guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
            }, 2000)
          ...
          await Upstaffs.updateOne({_id: belirle.id}, {$set: {"Mission": {
           ...
           if(logKanal) await logKanal.send({embeds: [new cartelinEmbedi().setDescription(`${guild.emojiGöster(emojiler.onay_munur)} ${belirle} üyesinin <t:${String(Date.now()).slice(0, 10)}:R> ${ramalcim} tarafından tüm görevleri temizlendi.`)]})
           await modal.reply({ephemeral: true, embeds: [new cartelinEmbedi().setDescription(`${guild.emojiGöster(emojiler.onay_munur)} Şuan da ${belirle} üyesine ait görev verileri temizleniyor...`)]})
           setTimeout(async () => {
               await modal.editReply({embeds: [new cartelinEmbedi().setDescription(`Başarıyla ${belirle} üyesine ait tüm görev verileri temizlendi. ${guild.emojiGöster(emojiler.onay_munur)}`)], ephemeral: true})
           }, 2000)
  ...
  onRequest: async function (client, message, args) {
    ...
    if(!sistem._rooter.rooters.includes(message.member.id) && !message.member.permissions.has("ADMINISTRATOR") && !roller.kurucuRolleri.some(x => message.member.roles.cache.has(x))) return message.reply(cevaplar.yetersiz).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    ...
    message.reply({embeds: [new cartelinEmbedi().setDescription(`Aşağıda bulunan düğmeler ile görev sistemine ekleme, güncelleme ve silme işlemlerini gerçekleştirebilirsin.`)], components: [Row]}).then(async (msg) => {
      ...
      collector.on('end', (i, reason) => {
        msg.delete().catch(err => {})
        if(reason == "time") message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
      })
    })
