...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let barSystem = [
      ...
      {id: "diger", başlamaBar: "munur_baslangicbar", doluBar: "munur_dolubar", doluBitişBar: "munur_dolubitisbar", boşBitişBar:"munur_bosbitis", boşBar: "munur_bosbar", başlangıçBar: "munur_bosbaslama"},
    ]
    ...
    if(ramalcim && (ayarlar && ayarlar.yetkiliYasaklıTag) && ayarlar.yetkiliYasaklıTag.filter(x => x != ayarlar.tag).some(x => ramalcim.user.username.includes(x))) return message.reply({
   ...
   // if(message.member.roles.cache.has(_statSystem.endstaff) && !message.member.permissions.has('ADMINISTRATOR')) return message.channel.send(`${cevaplar.prefix} \`Zaten son alt yetkidesin, bütün emeklerin için teşşekür ederiz.\``);
    const puanBilgisi = await Upstaffs.findOne({ _id: ramalcim.id })
    const davetbilgisi = await InviteData.findOne({userID: ramalcim.id}) || { regular: 0, bonus: 0, fake: 0, total: 0 };
   // const eskiRolcük = _statSystem.staffs[_statSystem.staffs.indexOf(_statSystem.staffs.find(x => ramalcim.roles.cache.has(x.rol) ))] || _statSystem.staffs[_statSystem.staffs.length-1];
   // const görevBilgisi = await Tasks.findOne({ Users: ramalcim.id }) || await Tasks.findOne({ roleID: eskiRolcük ? eskiRolcük.rol : ramalcim.roles.hoist.id  })
    Stats.findOne({ guildID: message.guild.id, userID: ramalcim.id }, async (err, data) => {
      if(!puanBilgisi) return message.reply(cevaplar.data).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
        ...
        const puanBari = _statSystem.staffs.some(x => ramalcim.roles.cache.has(x.rol)) && _statSystem.staffs.length > 0 ? `\`%${Math.floor((puanBilgisi ? puanBilgisi.Point.toFixed(1) : 0)/puanBelirt*100)}\` ${progressBar(puanBilgisi ? puanBilgisi.Point.toFixed(1) : 0, puanBelirt, 6, puanBilgisi.Point.toFixed(1))} \`${puanBilgisi ? puanBilgisi.Point.toFixed(1) : 0} / ${puanBelirt}\`` : "";
        let siralamabul = await Upstaffs.find({})
        let yetkilicezanotu = await ramalcim.yetkiliCezaPuan()
        ...
        const genelpuanBari = _statSystem.staffs.some(x => ramalcim.roles.cache.has(x.rol)) && _statSystem.staffs.length > 0 ? `\`%${Math.floor((puanBilgisi.ToplamPuan)/ToplamPuan*100)}\` ${progressBar(puanBilgisi ? puanBilgisi.ToplamPuan.toFixed(1) : 0, ToplamPuan, 6, puanBilgisi.ToplamPuan.toFixed(1))} \`${puanBilgisi ? puanBilgisi.ToplamPuan.toFixed(1) : 0} / ${ToplamPuan}\`` : "";
        
        let cezapuanoku = await ramalcim.cezaPuan()
        let teyitoku = await Users.findOne({ _id: ramalcim.id })
        let teyitbilgi;
        if(teyitoku) {
            if(teyitoku.Records) {
        ...
        let yetkidurumu;
        if(yeniRol) yetkidurumu = `Şu an <@&${eskiRol.rol}> rolündesiniz. bir sonraki <@&${yeniRol.rol}> ${yeniRol.exrol ? yeniRol.exrol.filter(x => !ramalcim.roles.cache.has(x)).length >= 1 ? `(${yeniRol.exrol.filter(x => !ramalcim.roles.cache.has(x)).map(x => message.guild.roles.cache.get(x)).join(",")}) rolerine` : "rolüne" : " rolüne"} ulaşmak için \`${Number(puanBelirt-puanBilgisi.Point).toFixed(1)}\` puan kazanmanız gerekiyor.`
        if(yeniRol.rol == eskiRol.rol) yetkidurumu = `Şu an son yetkidesiniz! Emekleriniz için teşekkür ederiz.`;
        if(roller.kurucuRolleri.some(x => ramalcim.roles.cache.has(x))) yetkidurumu = `Şu an son yetkidesiniz! Emekleriniz için teşekkür ederiz.`;
        let siralases = '';
        ...
        let Datacik = await Users.findOne({_id: ramalcim.id})
        ...
        let sonSes = SonGörülme ? SonGörülme.lastVoice ? ramalcim.voice && ramalcim.voice.channel ? `${ramalcim.voice.channel}` : `<t:${String(SonGörülme.lastVoice).slice(0, 10)}:R>` : `Tespit Edilmedi` : `Tespit Edilmedi`
        let siralamacik = 0
        let geçildic = 1
        siralamabul.filter(x => message.guild.members.cache.has(x._id)).sort((a, b) => b.ToplamPuan - a.ToplamPuan).forEach((veri, index) => {
          if(veri._id == ramalcim.id) siralamacik = index + 1
          if(veri._id != ramalcim.id) geçildic += 1
        })
         let embed = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(ramalcim.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${ramalcim} (${ramalcim.roles.highest}) kullanıcısının yetki ve yükseltim bilgileri aşağıda belirtilmiştir.`)
        .addFields(
...
{ name: `Yetki Durumu`, value: yetkidurumu }
        ...
        let puandetay = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(ramalcim.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${ramalcim} (${ramalcim.roles.highest}) kullanıcısının şuan ki yetkisinde kazandığı puan bilgileri aşağıda belirtilmiştir.`)
...
${simdises}`},
        ...
        let genelpuandurumu = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(ramalcim.user.avatarURL({dynamic: true, size: 2048})).setDescription(`${ramalcim} (${ramalcim.roles.highest}) kullanıcısı <t:${Number(String(Date.parse(puanBilgisi.Baslama)).substring(0, 10))}:F> tarihinde yetkili olmuş, kazandığı toplam puanlar ve detayları aşağıda belirtilmiştir.`).addFields(
...
{ name: `Genel Ses Sıralaması`, value: `${siralases ? siralases : `${message.guild.emojiGöster(emojiler.no_munur)} Maalesef genel ses puan detayları bulunamadığından listelenemedi.`}`},
{ name: `Yetki Durumu`, value: `Şu an <@&${eskiRol.rol}> rolünden, son ${message.guild.roles.cache.get(_statSystem.endstaff) ? message.guild.roles.cache.get(_statSystem.endstaff) : "@Rol Bulunamadı."} rolüne ulaşabilmek için **\`${Number(ToplamPuan-puanBilgisi.ToplamPuan).toFixed(1)}\`** puan kazanmanız gerekiyor.`}
     ...
     if(roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) || message.member.permissions.has("ADMINISTRATOR")) {
      buttonGroup.push([
        {label: "Yükseltim Paneli", description: `${ramalcim.user.tag} üyesinin yetki(yükselt/düşür) kontrolünü sağlar.`, emoji: {id: "621847216868425748"}, value: "buttonpanel"},
     ...
     let görevBul = await Tasks.findOne({ roleID: eskiRol ? eskiRol.rol : "1231131"  }) || await Tasks.findOne({ roleID: ramalcim.roles.hoist ? ramalcim.roles.hoist.id : "1231131" }) || await Tasks.findOne({ roleID: ramalcim.roles.highest ? ramalcim.roles.highest.id : "1231131" })
    let yönetimPaneli;
    let siralaman = 0
    let geçildi = 1
    siralamabul.filter(x => message.guild.members.cache.has(x._id)).sort((a, b) => b.Görev - a.Görev).forEach((veri, index) => {
      if(veri._id == ramalcim.id) siralaman = index + 1
      if(veri._id != ramalcim.id) geçildi += 1
    })
    let public = 0;
    let register = 0;
    let genelses = 0;
    ...
    if(görevBul) {
      ...
      if(data.taskVoiceStats) {
        data.taskVoiceStats.forEach((value, key) => {
            genelses += value
            if(key == kanallar.publicKategorisi) public += Number(client.munurSaatYap(value))
            if(key == kanallar.streamerKategorisi) public += Number(client.munurSaatYap(value))
            if(key == kanallar.registerKategorisi) register += Number(client.munurSaatYap(value))
        });
      };
      ...
      if(görevBul.Time && görevBul.Time > 0 && Date.now() >= görevBul.Time) {
        ...
        yönetimPaneli = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048}))
        .setDescription(`${ramalcim} (${görevBul.roleID ? message.guild.roles.cache.get(görevBul.roleID) : "@Rol Bulunamadı."}) ${görevBul.Completed.includes(ramalcim.id) ? `üyesinin rolüne ait görev tamamlama zamanı dolmuş ve tamamlamış diğer bilgiler ise aşağıda belirtilmiştir.` : `üyesinin rolüne ait görev tamamlama zamanı dolmuş fakat tamamlayamamış diğer bilgiler ise aşağıda belirtilmiştir.`}`)
...
${görevBul.Time ? `\`Görev Bitiş Zamanı        :\`  Bitti!\n` : ``}\`Durum                     :\` ${durumcuuukk}`})
      ...
      yönetimPaneli = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048})).setThumbnail(ramalcim.user.avatarURL({dynamic: true, size: 2048}))
    .setDescription(`${ramalcim}, (${görevBul.roleID ? message.guild.roles.cache.get(görevBul.roleID) : "@Rol Bulunamadı."}) üyesinin yetkili bilgisi aşağıda detaylı bir şekilde belirtilmiştir. Bulunan düğmeler ile aktif görevinin bilgisini görüntüleyebilirsin.`)
    .addFields({ name: `Yetkili Bilgisi`, value: `\`Rol                       :\` ${Number(bulundugurol) + 1}. ROL
\`Sıralama                  :\` #${siralaman} (Durum: ${siralaman}/${geçildi})
\`Rolde Kalma Süresi        :\` ${client.sureCevir(Date.now() - puanBilgisi.Rolde)}
\`Yetkide Kalma Süresi      :\` ${client.sureCevir(Date.now() - puanBilgisi.Baslama)}
...
\`Durum                     :\` Şuan da aktif görev bulunamadı.
          ...
          `})
        .setDescription(`${ramalcim} (${eskiRol ? message.guild.roles.cache.get(eskiRol.rol) : ramalcim.roles.highest}) üyesinin aşağıda yetki ve görev bilgileri belirtilmektedir.`)
      }
    let compo = []
    if(görevBul && görevBul.Time && görevBul.Time > 0 && Date.now() >= görevBul.Time) {
      if(görevBul.Completed.includes(ramalcim.id)) {
        if(yeniRol && yeniRol.rol && !roller.kurucuRolleri.some(x => ramalcim.roles.cache.has(x)) && !ramalcim.roles.cache.has(_statSystem.endstaff) && eskiRol.rol != roller.başlangıçYetki) yönetimPaneli.addFields({name: `**Yükseltim Durumu**`, value: `Bir sonraki <@&${yeniRol.rol}> ${yeniRol.exrol ? "(" + yeniRol.exrol.map(x => message.guild.roles.cache.get(x)).join(",") + ")" : ""} rolüne yükselmek için gereken tüm görevleri tamamladı. Yükseltim yapmak için aşağıda bulunan "Yükselt" düğkmesini kullanabilirsin.`})
        compo = [
          new MessageButton()
          .setCustomId("yükselt")
          .setLabel("Yükselt")
      ...
      if(yeniRol && yeniRol.rol && !roller.kurucuRolleri.some(x => ramalcim.roles.cache.has(x)) && !ramalcim.roles.cache.has(_statSystem.endstaff) && eskiRol.rol != roller.başlangıçYetki) yönetimPaneli.addFields({name: `**Yükseltim Durumu**`, value: `Şu an <@&${eskiRol.rol}> rolündesiniz. Bir sonraki <@&${yeniRol.rol}> ${yeniRol.exrol ? "(" + yeniRol.exrol.map(x => message.guild.roles.cache.get(x)).join(",") + ")" : ""} rolüne yükselmek için tüm görevleri tamamlamalısınız.`})
      compo = [
        new MessageButton()
        .setCustomId("yetki_bilgisi")
        .setLabel("Yetki Bilgisi")
    ...
    let RowGorev = new MessageActionRow().addComponents(
      compo
    )
    let RowCukiki = new MessageActionRow().addComponents(
      new MessageButton()
      .setCustomId("yetki_bilgisi")
      .setDisabled(true)
      .setLabel("Yetki Bilgisi")
    ...
    if(puanBilgisi && (görevBul || puanBilgisi.Yönetim)) return await message.reply({embeds: [yönetimPaneli], components: roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) || message.member.permissions.has("ADMINISTRATOR") ? [RowGorev] : [RowCukiki]}).then(async (manmsg) => {
      ...
      collector.on("collect", async (button) => {
        if(button.customId === "gorev_bilgisi") {
          let embedcik = new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true, size: 2048})).setDescription(`Aşağı da ${görevBul.roleID ? message.guild.roles.cache.get(görevBul.roleID) : "@Rol Bulunamadı."} rolüne ait yapılması gereken görevler belirtilmiştir. Görev süreniz boyunca en az **%75**'lik kısmını tamamlamanız gerekmektedir.`)
          if(görevBul.publicVoice >= 1) {
            if(!roller.teyitciRolleri.some(x => ramalcim.roles.cache.has(x))) embedcik.addFields({ name: `${görevBul.publicVoice} Saat Public/Streamer Kanalında Takıl!`, value: `${public >= görevBul.publicVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Ses) } ${yönetimBar(public ? public : 0, görevBul.publicVoice, 5, public)} \`${public >= görevBul.publicVoice ? `Tamamlandı!`: `${public} saat / ${görevBul.publicVoice} saat`}\``})
            if(roller.teyitciRolleri.some(x => ramalcim.roles.cache.has(x))) embedcik.addFields({ name: `${görevBul.publicVoice} Saat Register Kanallarında Takıl!`, value: `${register >= görevBul.publicVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Ses) } ${yönetimBar(register ? register : 0, görevBul.publicVoice, 5, register)} \`${register >= görevBul.publicVoice ? `Tamamlandı!`: `${register} saat / ${görevBul.publicVoice} saat`}\``})
           }
           if(görevBul.AllVoice >= 1) {
             embedcik.addFields({ name: `${görevBul.AllVoice} Saat Tüm Ses Kanallarında Takıl!`, value: `${Number(client.munurSaatYap(genelses)) >= görevBul.AllVoice ? message.guild.emojiGöster(emojiler.Görev.OK) : message.guild.emojiGöster(emojiler.Görev.Mesaj) } ${yönetimBar(Number(client.munurSaatYap(genelses)) ? Number(client.munurSaatYap(genelses)) : 0, görevBul.AllVoice, 5, Number(client.munurSaatYap(genelses)))} \`${Number(client.munurSaatYap(genelses)) >= görevBul.AllVoice ? `Tamamlandı!`: `${Number(client.munurSaatYap(genelses))} saat / ${görevBul.AllVoice} saat`}\``})
           }
          ...
          manmsg.edit({embeds: [embedcik], components: [ roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) || message.member.permissions.has("ADMINISTRATOR") ? RowGorev : RowCukiki]})
        ...
        manmsg.edit({embeds: [yönetimPaneli], components: [ roller.kurucuRolleri.some(x => message.member.roles.cache.has(x)) || message.member.permissions.has("ADMINISTRATOR") ? RowGorev : RowCukiki]})
          await button.deferUpdate()
        }
