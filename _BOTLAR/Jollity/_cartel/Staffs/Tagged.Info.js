...
module.exports = {
  ...
  Kullanim: "taglıbilgisi <@munur/ID>",
 ...
 .setStyle('DANGER');
    ...
    Users.findOne({_id: ramalcim.id }, async (err, res) => {
      if (!res) return message.channel.send({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli üyenin taglı bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      if(!res.Taggeds) return message.channel.send({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.avatarURL({dynamic: true})).setDescription(`${ramalcim} isimli üyenin taglı bilgisi bulunamadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      ...
      const curPage = await message.channel.send({
        embeds: [embed.setDescription(`${ramalcim}, üyesin taglı bilgisi yükleniyor... Lütfen bekleyin...`)],
        components: [row], fetchReply: true,
      }).catch(err => {});
    
      await curPage.edit({embeds: [embed.setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Taggeds.length || 0}\` adet taglısı mevcut.
...
${pages[currentPage - 1].map((value, index) => `\` ${index + 1} \` ${message.guild.members.cache.get(value.id) ? message.guild.members.cache.get(value.id) : `<@${value.id}>`}  <t:${String(value.Date).slice(0, 10)}:R>`).join("\n")} `)]}).catch(err => {})
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Taggeds.length || 0}\` adet taglısı mevcut.
...
${pages[currentPage - 1].map((value, index) => `\`${index + 1}.\` ${message.guild.members.cache.get(value.id) ? message.guild.members.cache.get(value.id) : `<@${value.id}>`} \`${tarihsel(value.Date)}\``).join("\n")}`)]
      ...
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, message.guild.iconURL({dynamic: true})).setDescription(`${ramalcim} isimli üyesinin toplamda \`${res.Taggeds.length || 0}\` adet taglısı mevcut.`)],
          components: [],
        }).catch(err => {});
      })
    })
