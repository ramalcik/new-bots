...
if(!Upstaffs ) {
    ...
    if(!rolBul) return message.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} ${ramalcim} isimli üye son yetkiye ulaştığı için yükseltme işlemi yapılamaz.`, ephemeral: true})
    if(rolBul.No > ayarlar.yükseltimSınırı && !roller.kurucuRolleri.some(oku => message.member.roles.cache.has(oku)) && !message.member.permissions.has('ADMINISTRATOR') && !roller.yükselticiRoller.some(x => message.member.roles.cache.has(x)) && roller.limitliYükselticiRolleri.some(x => message.member.roles.cache.has(x))) return message.reply({content: `${message.guild.emojiGöster(emojiler.no_munur)} ${ramalcim}, isimli üyeyi en fazla şuan ki yetkiye kadar yükseltebilirsin.`, ephemeral: true}).then(x => setTimeout(() => {
        x.delete().catch(err => {})
    }, 7500))
...
await Upstaff.updateOne({ _id: ramalcim.id }, { $set: { "Point": 0, "staffNo": Number(rolBul.No) + Number(1), "staffExNo": rolBul.No, "Yetkili": 0, "Görev": 0, "Invite": 0,  "Tag": 0, "Register": 0, "Ses": new Map(), "Mesaj": 0, "Bonus": 0, "Toplantı": 0, "Etkinlik": 0 }}, {upsert: true});
      message.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla ${ramalcim} isimli yetkili, \`${message.guild.roles.cache.get(rolBul.rol).name}\` rolüne yükseltildi.`, ephemeral: true }).then(x => {
        setTimeout(() => {
            x.delete()
        }, 7500);
