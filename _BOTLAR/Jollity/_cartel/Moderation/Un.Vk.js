...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await VK.findById(ramalcim.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: ramalcim.id, Active: true, Type: "VK Cezalandırma" }) 
    ...
    if(await VK.findById(ramalcim.id)) {
        await VK.findByIdAndDelete(ramalcim.id)
    }
    if(ramalcim && ramalcim.manageable) await ramalcim.roles.remove(roller.vkCezalıRolü).catch(err => {});;
