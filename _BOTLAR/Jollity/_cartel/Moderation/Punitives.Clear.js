...
module.exports = {
 ...
 if(cezabul && cezabul.Active) return message.channel.send(`${cevaplar.prefix} Belirtilen (\`#${args[0]}\`) ceza numarasında aktif ceza bulunduğundan dolayı işlem iptal edildi.`).then(s => setTimeout(() => s.delete().catch(err => {}), 5000));
    await message.channel.send({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} ${cezabul.Member ? message.guild.members.cache.get(cezabul.Member) ? message.guild.members.cache.get(cezabul.Member) : `<@${cezabul.Member}>` : `<@${cezabul.Member}>`} isimli üyeye ait \`#${cezabul.No}\` numaralı ceza sicilden temizlendi.`)]})
