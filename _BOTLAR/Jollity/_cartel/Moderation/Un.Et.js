...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await ETKINLIK.findById(ramalcim.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: ramalcim.id, Active: true, Type: "Etkinlik Cezalandırma" }) 
    ...
    if(await ETKINLIK.findById(ramalcim.id)) {
        await ETKINLIK.findByIdAndDelete(ramalcim.id)
    }
    if(ramalcim && ramalcim.manageable) await ramalcim.roles.remove(roller.etkinlikCezalıRolü).catch(err => {});;
