...
module.exports = {
 ...
 .setStyle('DANGER');
    ...
    const button2 = new MessageButton()
      ...
      if (!res) return message.reply({embeds: [new cartelinEmbedi().setDescription(`${message.guild.name} sunucusunda ceza uygulanmadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      if(!res) return message.reply({embeds: [new cartelinEmbedi().setDescription(`${message.guild.name} sunucusunda ceza uygulanmadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      let data = []
      ...
      console.log(data)
      var currentPage = 1
      if (!pages && !pages.length || !pages[currentPage - 1]) return message.reply({embeds: [new cartelinEmbedi().setDescription(`${message.guild.name} sunucusunda ceza uygulanmadı.`)]}).then(x => setTimeout(() => {x.delete()}, 7500))
      let embed = new cartelinEmbedi().setColor("RANDOM").setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length}`, message.guild.iconURL({dynamic: true}))
      ...
      collector.on("collect", async (i) => {
        ...
        await i.deferUpdate();
        await curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name} • ${currentPage} / ${pages.length} `, message.guild.iconURL({dynamic: true})).setDescription(`${pages[currentPage - 1].map((x, index) => `\` ${x.No} \` <@${x.Member}> <t:${String(Date.parse(x.Date)).slice(0, 10)}:R> [**${x.Type}**] (<@${x.Staff}>)`).join("\n")}`)]
        }).catch(err => {});
        collector.resetTimer();
      });
      collector.on("end", () => {
        if(curPage) curPage.edit({
          embeds: [embed.setFooter(`${ayarlar.serverName ? ayarlar.serverName : message.guild.name}`, message.guild.iconURL({dynamic: true})).setDescription(`${message.guild.name} sunucusunda toplamda \`${res.length || 0}\` adet ceza uygulandı.`)],
          components: [],
        }).catch(err => {});
      })
    }
