...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    .setCustomId(`onayla`)
    .setLabel(await ETKINLIK.findById(ramalcim.id) ? `Aktif Cezalandırılması Mevcut!` : 'Cezalandırmayı Onaylıyorum!')
    .setEmoji(message.guild.emojiGöster(emojiler.Cezalandırıldı))
    .setStyle('SECONDARY')
    .setDisabled(await ETKINLIK.findById(ramalcim.id) ? true : false )
    ...
    let msg = await message.reply({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${ramalcim} isimli üyeyi etkinliklerden cezalandırmak istiyor musun?`)], components: [jailOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        if (i.customId === `onayla`) {
            i.update({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${ramalcim} isimli üyesini hangi sebep ile etkinliklerden cezalandırmak istiyorsun?`)], components: [new MessageActionRow().addComponents(
