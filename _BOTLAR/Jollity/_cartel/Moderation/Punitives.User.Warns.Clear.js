...
module.exports = {
    ...
    Kullanim: "uyarıtemizle <@munur/ID>",
  ...
  onRequest: async function (client, message, args) {
    ...
    if(await Punitives.findOne({Member: ramalcim.id, Type: "Uyarılma"})) {
    	await message.reply({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} ${ramalcim} üyesinin tüm uyarılmaları başarıyla temizlendi.`)]})
    	await Punitives.updateMany({Member: ramalcim.id, Type: "Uyarılma" }, { $set: { Member: `Silindi (${ramalcim.id})`, No: "-99999", Remover: `Sildi (${message.author.id})`} }, { upsert: true });
	...
	await message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
	return message.reply({embeds: [new cartelinEmbedi().setDescription(`${ramalcim} isimli üyenin uyarısı bulunamadığından dolayı işlem iptal edili.`)]});
    }
    }
};