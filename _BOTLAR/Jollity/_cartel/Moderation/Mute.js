...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let msg = await message.reply({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirtilen ${ramalcim} isimli üyeyi hangi türde susturmak istiyorsun?`)], components: [muteOptions]}).catch(err => {})
    ...
    collector.on('collect', async i => {
        ...
        selectMute = 5
        i.update({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${ramalcim} isimli üyesini hangi sebep ile **metin kanallarından** susturmamı istiyorsun?`)], components: [new MessageActionRow().addComponents(
        ...
        if (i.customId === `voicemute`) {
            selectMute = 4
            i.update({embeds: [new cartelinEmbedi().setAuthor(ramalcim.user.tag, ramalcim.user.displayAvatarURL({dynamic: true})).setDescription(`Belirlenen ${ramalcim} isimli üyesini hangi sebep ile **ses kanallarından** susturmamı istiyorsun?`)], components: [new MessageActionRow().addComponents(
                new MessageSelectMenu()
                .setCustomId(`sebep`)
                .setPlaceholder(`Susturmak istediğiniz sebebi seçinin.`)
                .addOptions([
		...
		let voiceMuteCheck = await voiceMute.findById(ramalcim.id)
		if(voiceMuteCheck) return await i.reply({content: `Belirtiğin ${ramalcim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
               ...
               if(selectMute == 5) {
		let chatMuteCheck = await Mute.findById(ramalcim.id)
		if(chatMuteCheck) return await i.reply({content: `Belirtiğin ${ramalcim} üyesinin, aktif bir susturulma cezası mevcut!`, ephemeral: true}),msg.delete().catch(err => {}),message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
