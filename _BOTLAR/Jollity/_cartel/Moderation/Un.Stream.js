...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    let cezakontrol = await STREAM.findById(ramalcim.id) 
    if(!cezakontrol) {
    ...
    let cezabilgisi = await Punitives.findOne({ Member: ramalcim.id, Active: true, Type: "Streamer Cezalandırma" }) 
    ...
    if(await STREAM.findById(ramalcim.id)) {
        await STREAM.findByIdAndDelete(ramalcim.id)
    }
    if(ramalcim && ramalcim.manageable) await ramalcim.roles.remove(roller.streamerCezalıRolü).catch(err => {});;
