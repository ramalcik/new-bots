...
const { Client, Message, MessageAttachment } = Discord = require("discord.js");
...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
        ...
        let back = await loadImage(`../../_BASE/img/back.png`);
        let one = await loadImage("../../_BASE/img/one.png"); 
        let two = await loadImage("../../_BASE/img/two.png");  
        let three = await loadImage("../../_BASE/img/three.png");      
        let icon = await loadImage(`../../_BASE/img/voice.png`)
        let icon2 = await loadImage("../../_BASE/img/chat.png")
        let image = new Canvas(697, data.length * 59)
        let start = 30
        let bstart = 0
        for (let i = 0; i < data.length; i++) {
            let ramalcim = message.guild.members.cache.get(data[i].id)
            let avatar = await loadImage(ramalcim.user.avatarURL({format: "jpg"}) ? ramalcim.user.avatarURL({format: "jpg"}) : "https://cdn.discordapp.com/attachments/990322473750917120/1005299390921064498/aotNervousEren.png");
            ...
            if(ramalcim.user.tag.toString().length > 12) {
                image.setTextFont('18px Arial Black'), image.printText(`${ramalcim.user.tag}`, 130, Number(start + 5), 850)
            } else {
                image.setTextFont('20px Arial Black'), image.printText(`${ramalcim.user.tag}`, 130, Number(start + 2), 850)
            }
        ...
        let attachment = new MessageAttachment(image.toBuffer(), "munur-siralama.png");
    

        let ses = new cartelinEmbedi()
        ...
        .setImage("attachment://munur-siralama.png")
