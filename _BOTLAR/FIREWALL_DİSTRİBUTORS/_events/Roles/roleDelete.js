...
module.exports = async role => {
  ...
  if(Data && !Data.roleGuard) return;
    let embed = new cartelinEmbedi().setTitle("Sunucuda Rol Silindi!")
...
**Tahmini Dağıtım Süresi**: \` ${roleData ? roleData.members.length > 0 ? (roleData.members.length>1000 ? parseInt((roleData.members.length*(250/1000)) / 60)+" dakika" : parseInt(roleData.members.length*(250/1000))+" saniye") : "Belirlenemedi." : "Belirlenemedi." } \``).setFooter("yukarda belirtilen rol bilgisi dahilinde tekrardan kurulum sağlanmıştır ve dağıtımı başlamıştır.");
