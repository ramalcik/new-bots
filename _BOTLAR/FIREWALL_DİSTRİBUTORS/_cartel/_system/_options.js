...
const Heykeller = require('../../../../_SYSTEM/Databases/Schemas/Others/Middle.Heykels')
let statConfig;
...
module.exports = {
  ...
  onLoad: function (client) {
    client.on('modalSubmit', async (modal) => {
      statConfig =  require('../../../../_SYSTEM/Additions/Staff/Sources/_settings')
      ...
      if(modal.customId == "istekoneri") {
        ...
        let embed = new cartelinEmbedi().setFooter(`${cartelim.user.tag} • Yeni ${ayarlar.serverName} İstek/Öneri`, cartelim.user.avatarURL({dynamic: true}))
        let Etiket;
...
${cartelim} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda istek veya önerisi belirtilmiştir.`)
        ...
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla istek veya öneriniz yönetime iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      }
      if(modal.customId == "botsorun") {
        ...
        let embed = new cartelinEmbedi().setFooter(`${cartelim.user.tag} • Yeni Bot Sorun Bildirimi`, cartelim.user.avatarURL({dynamic: true}))
...
${cartelim} isimli üyenin <t:${String(Date.now()).slice(0,10)}:F> tarihinde aşağıda bot sorunu bildirdi.`)
        ...
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla bot sorunu münür'a iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      }
      ...
      //  }
        ...
        let embed = new cartelinEmbedi().setFooter(`${cartelim.user.tag} • Yeni ${ayarlar.serverName} Sorun Çözme Çağırma Formu`, cartelim.user.avatarURL({dynamic: true}))
        logKanalı.send({content: `${roller.sorunÇözmeciler.map(x => cartelim.guild.roles.cache.get(x)).join(", ")}`, embeds: [embed.setDescription(`${cartelim} isimli cezalı bir üye sorun çözme çağırmak istiyor. Aktif olan sorun çözmecilerimizin bu olaya bakmasını istiyorum.`)
      .addField("Sorun Tipi",`> ${sorun}`)
      .addField("Sorun",`> ${hakkında}`)
    ]})
    await modal.deferReply({ ephemeral: true })
    await modal.followUp({content: `Başarıyla sorun çözmeye hatalı bildiri iletilmiştir. Teşekkür Ederiz! ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      }
      if(modal.customId == "ybasvuru") {
        ...
        let embed = new cartelinEmbedi().setFooter(`${cartelim.user.tag} • Yeni ${ayarlar.serverName} Yetkili Başvurusu`, cartelim.user.avatarURL({dynamic: true}))
        let Etiket;
...
.addField(`Hakkında`, `${hakkında}`)
        ...
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla yetkili başvuru kaydınız alınmıştır en kısa süreçte sizlere ulaşacağız, lütfen özel mesaj kutunuzu herkese açık yapın. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      }  
  });
    ...
    client.ws.on('INTERACTION_CREATE', async interaction => {
      ...
      if(name == "bestFriend") {
        ...
        if(!heykelKontrol) {
          returnText = `**Üzgünüm!** Yakın arkadaş  listesine girebilmek için listeye eklenmen gerekli. Lütfen daha sonra tekrar deneyin! ${member.guild.emojiGöster(emojiler.no_munur)}`
        } else if(heykelKontrol) {
          if(roller.Buttons.bestFriendRolü && member.roles.cache.has(roller.Buttons.bestFriendRolü)) {
            returnText = `${member.guild.emojiGöster(emojiler.no_munur)} **Alınmış!** Daha önce alındığı için 00:00 Saatini beklemelisin.`
            ...
            returnText = `**Başarılı!** Artık Sende Bir Yakın Arkadaş Oldun!  ${member.guild.emojiGöster(emojiler.onay_munur)}`
          }
        }
...
aktifCezalarList.map(x => `${member.guild.emojiGöster(emojiler.no_munur)} ${x.Yetkili} tarafından **${x.Atılan}** süresince işlenen "__#${x.No}__" numaralı "__${x.Tip}__" türündeki cezalandırmanın kalkmasına **${x.Kalkma}** kaldı.`).join("\n") 
: `${member.guild.emojiGöster(emojiler.onay_munur)} Tebrikler! \`${member.guild.name}\` sunucusunda size ait aktif aktif cezaya ulaşılamadı.`}`
      }
      ...
      client.api.interactions(interaction.id, interaction.token).callback.post({
          data: {
              type: 4,
              data: {
                  content: returnText ? returnText : `${member.guild.emojiGöster(emojiler.onay_munur)} Tebrikler! \`${member.guild.name}\` sunucusunda size ait aktif cezaya ulaşılamadı.`,
                  flags: "64"
              }
          }
      })
    ...
    client.on('interactionCreate', async (i) => {
      ...
      if(i.customId == "cdestekcik") {
        ...
        await i.reply({ embeds:[new cartelinEmbedi().setDescription(`Canlı desteğe bağlanmak istediğinize emin misiniz?`).setFooter(`gereksiz isteklerde yaptırım uygulanacaktır.`)] , components: [row2], ephemeral: true});
        ...
        collector.on('collect', async (collect) => {
          if(collect.customId == "evt") {
            await i.editReply({embeds: [new cartelinEmbedi().setDescription(`Canlı destek ekibimize bildirdik, sizi canlı destek ekibine aktarıyorum. Lütfen bekleyin!`)], components: [], ephemeral: true});
            let logKanalı = i.guild.kanalBul("canlı-destek")
            if(logKanalı) logKanalı.send({content: `${canlıDestekRolü.map(x => i.guild.roles.cache.get(x)).join(", ")}`, embeds: [new cartelinEmbedi().setDescription(`${member} üyesi canlı desteğe bağlanmak istiyor. Kabul ediyor musunuz?`)], components: [row3]}).then(async (msg) => {
              var filter = (i) => {
              ...
              collector2.on("collect", async (interaction) => { 
                if(interaction.customId == "onayla") {
                  msg.edit({
                    content: null,
                    embeds : [new cartelinEmbedi().setDescription(`${member} üyesinin canlı desteği <t:${String(Date.now()).slice(0,10)}:F> tarihinde ${interaction.user} tarafından onaylandı. ${member.guild.emojiGöster(emojiler.onay_munur)}`)],
                    components : []
                  })
                  ...
                  member.guild.channels.create(`${member.user.tag}-destek`, {
                    ...
                    c.send({
                      embeds: [new cartelinEmbedi().setDescription(`Canlı destek kanalı başarıyla oluşturuldu.
**NOT:** Canlı destek almaktan vaz geçerseniz veya destek bitti ise aşağıda ki düğmeyi kullanabilirsiniz.`).setFooter(`bu canlı destek 5 dakika sonra kapatılacaktır.`)],
                      ...
                      collectorcuk.on('collect', async (inte) => {
                        if(inte.customId == "kapatCanliDestek") {
                          ...
                          cmsg.edit({embeds: [new cartelinEmbedi().setDescription(`${cmsg.guild.emojiGöster(emojiler.onay_munur)} ${inte.user} tarafından canlı destek kapatıldı. 10 Saniye içerisinde kanal yok olacaktır.`)],components: []})
                          setTimeout(() => {
                            c.delete().catch(err => {})
                          }, 10000);
                        }
                ...
                if(interaction.customId == "reddet") {
                  ...
                  msg.edit({content: null, embeds: [new cartelinEmbedi().setDescription(`${cevaplar.prefix} ${member} üyesinin canlı destek isteği <t:${String(Date.now()).slice(0, 10)}:R> ${interaction.user} tarafından reddedildi.`)], components: []}).catch(err => {})
                  await interaction.reply({ephemeral: true, content: `${member.guild.emojiGöster(emojiler.onay_munur)} Başarıyla ${member} üyesinin, canlı desteğini iptal ettin.`}) 
                  setTimeout(() => {
      ...
      if(i.customId == "sorunÇözmeci") {
       ...
       if(i.customId == "basvurucuk") {
          ...
          .addComponents(
            ...
            .setMinLength(5)
            .setMaxLength(100)
            .setPlaceholder('Örn: münür#0001/ID')
            .setRequired(false),
            new TextInputComponent()
...
**Örnek Görsel**`, image: "https://cdn.discordapp.com/attachments/919637452648493116/942309234245263391/unknown.png", category: "taglı"})
              ...
              let kategori = i.values
               ...
               if(getInfo) {
                   let embed = new cartelinEmbedi().setTitle(getInfo.header).setDescription(getInfo.description)
  ...
  onRequest: async function (client, message, args) {
        ...
        let ozelliklerListe = Object.keys(data || {}).filter(a => özellikler.find(v => v.name == a))
         ...
         let Rowck = new MessageActionRow().addComponents(
          ...
          .setOptions(
              ...
              {label: "Rol Alma", emoji:"1035614878142038076", description: "Sunucuda Etkinlik Çekiliş Rollerini Almaya Yarayan Paneli Kurar.", value: "etkinlikçekilişkur"},
              {label: "Rol Kurulumu", emoji:"943285259733184592", description: "Sunucudaki Gerekli Olan Rolleri/Sistemleri Kurar.", value: "otomatikkur"},
              {label: "Kısayollar",  emoji:"1056279166590394408", description:  "Botlarda Bulunan Komutları Listelemek İçin Sistemi Kurar.", value: "yardimkur"},
              {label: "Kullanıcı Paneli", emoji:"1056279162534494300", description: "Sunucudaki Üyelerin Yardım Alması İçin Gerekli Sistemi Kurar.", value: "kpaneli"},
              {label: "Kullanıcı Paneli 2", emoji:"1056279162534494300", description: "Sunucudaki Üyelerin Yardım Alması İçin Gerekli Sistemi Kurar.", value: "kullanicip2"},
              {label: "Ceza Paneli", emoji:"951514358377234432", description: "Sunucudaki Üyelerin Cezalarını/Ceza Puanlarını Öğrenmeye Yarayan Sistemi Kurar.", value: "cezapanelikur"},
              {label: "Yetkili Paneli", emoji:"1048546943460180028", description: "Sunucudaki Yetkililerin İşlemleri Hakkında Bilgi Verem Sistemi Kurar.", value: "yetkiliPaneli"},
              {label: "Yönetim Paneli", emoji:"1048543653976621057", description: "Sunucudaki Yönetim Kadrosunda Bulunan Kişilerin İşlemleri Hakkında Bilgi Verem Sistemi Kurar.", value: "yöneticiPaneli"},
              {label: "Yakın Arkadaşlık", emoji:"1063607983726133258", description: "Sunucudaki Üyelerin Birbirleriyle Kaynaşması/Dost Olmasına Yarayan Sistemdir", value: "kisayol"},
              )
        ...
        const sywss = new MessageActionRow().addComponents(
          new MessageSelectMenu()
          .setCustomId("qwewqwq")
          .setPlaceholder("🎄 Creates")
          .setOptions(
          {label: "Etkinlik & İlişki Rolleri", emoji:"1056279166590394408", description: "Sunucudaki Üyelerin Etkinlik/Çekiliş'lerden Faydalanması İçin Gerekli Rolleri Kurar", value:"otomatikilişki"},
          {label: "Renk Rolleri", emoji:"1056279166590394408", description: "Sunucudaki Üyelerin Renk Rollerini Alması İçin Gerekli Renk Rollerini Kurar.", value: "otomatikrenk"},
          {label: "Oyun Rolleri", emoji:"1056279166590394408", description: "Sunucudaki Üyelerin Oyun Rollerini Alması İçin Gerekli Oyun Rollerini Kurar", value: "otomatikoyun"},
          {label: "Burç Rolleri", emoji:"1056279166590394408", description: "Sunucudaki Üyelerin Burç Rollerini Alması İçin Gerekli Burç Rollerini Kurar", value: "otomatikburç"},
          {label: "Aylık Üye Rolleri", emoji:"1056279166590394408", description: "Sunucudaki Üyelerin Serverde Geçirdiği Zamana Göre Rollerini Verir", value: "otomatikaylikcartelim"},
          {label: "Ses Rozet Rolleri",  emoji:"1056279166590394408", description:  "Kullanıcıların Seste Durduğu Kadar Üzerilerine Rol Eklenir.", value: "otomatiksesrozet"},
...
`)
            return message.channel.send({components: [Rowck],embeds: [emboo]}).then(async (x) => {
                ...
                collector.on('collect', async i => {
                  if(i.values[0] == "yardimkur") {
                    let Rowck = new MessageActionRow().addComponents(
                      new MessageSelectMenu()
                      .setCustomId("münür_yardim")
                  ...
                  allBots.forEach(async (cl) => {
                    if(cl.token == sistem.TOKENLER.Jollity) {
                  ...
                  if(i.values[0] == "otomatiksesrozet") {
                    ...
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik ses rozet rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let aylıkSistem = [
                  ...
                  if(i.values[0] == "otomatikaylikcartelim") {
                    if(ayarlar.aylikcartelim && ayarlar.birAy && message.guild.roles.cache.get(ayarlar.birAy)) return await i.reply({content: `${cevaplar.prefix} Otomatik kurulum sistemini aktif etmek için, aktif bir rol bulunmaması gerekir.`, ephemeral: true});
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik aylık üye rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let aylıkSistem = [
                  ...
                  if(i.values[0] == "otomatikburç") {
                    ...
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik burç rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let burçSistemi = [
                  ...
                  if(i.values[0] == "otomatikilişki") {
                    ...
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik ilişki & etkinlik rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let iliskiSistemi = [
                  ...
                  if(i.values[0] == "otomatikoyun") {
                    ...
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik oyun rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let oyunSistemi = [
                  ...
                  if(i.values[0] == "otomatikrenk") {
                    ...
                    await i.reply({content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla otomatik renk rolleri oluşturularak veritabanına kayıt edildi.`, ephemeral: true})
                    await x.delete().catch(err => {})
                    let renkSistemi = [
                  ...
                  if(i.values[0]  == "kisayol") {
                    await x.delete().catch(err => {}),await i.deferUpdate().catch(err => {}).catch(err => {}); 
                    client.api.channels(message.channel.id).messages.post({ data: {"content":`**Merhaba!** Sende **${ayarlar.serverName}** sunucusunda yakın arkadaşımız mısın? ${message.guild.emojiGöster(emojiler.serverTag)} 
...
Unutma her gün sonunda o rol üzerinden sistem olarak kaldırılmaktadır. Bu kanaldan tekrardan alabilirsin. :tada: :tada: :tada:`,"components":[{"type":1,"components":[
                      ...
                      await message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {})
                  }
...
await message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {})
                  ...
                  if(i.values[0]  == "yöneticiPaneli") {
                    ...
                    x.delete().catch(err => {})
                    await i.deferUpdate().catch(err => {}),await message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {})
                  }
                  if(i.values[0]  == "yetkiliPaneli") {
                    ...
                    if(i.values[0]  === "cezapanelikur") {
                      await x.delete().catch(err => {}),await i.deferUpdate().catch(err => {}).catch(err => {}); 
                        client.api.channels(message.channel.id).messages.post({ data: {"content":`**Merhaba!** ${ayarlar.serverName} ${message.guild.emojiGöster(emojiler.serverTag)}
...
Sorun çözmeciye cezanızı itiraz mı etmek istiyorsunuz? "Sorun Çözme Bildir!" düğmesi ile bildirebilirsiniz.`,"components":[{"type":1,"components":[
                    ...
                    if(i.values[0]  == "otomatikkur") {
                      await x.edit({content: null, components: [ sywss ], embeds: [new cartelinEmbedi().setDescription(`
                                               ...
                                               
  {
    "type": 1, "components": [{
            ...
            { "label": "Çekiliş Katılımcısı", "description": "Çekilişlerden haberdar olmak için", "value": "cekilis", "emoji": { "id": "922059128250195978", "name": "monarch_cekilis" }, },
                      ...
                      if(ayarlar.Buttons && ayarlar.Buttons.alone && message.guild.roles.cache.get(ayarlar.Buttons.alone)) client.api.channels(message.channel.id).messages.post({
                        data: {
                            ...
                            await message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {}).catch(err => {})
                    }
        ...
        if (ozellik.type) {
            ...
            if(!rol) return message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.no_munur)} **${başHarfBüyült(ozellik.name)}** isimli seçenek ayarını hangi rol yapmamı istiyorsun?`)]}).then(x => setTimeout(() => {
              x.delete().catch(err => {});
          }, 7500));
            await GUILD_SETTINGS.findOneAndUpdate({guildID: message.guild.id}, {$set: {[`Ayarlar.Buttons.${ozellik.name}`]: rol.id}}, {upsert: true}).catch(e => console.log(e))
             message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla **${başHarfBüyült(ozellik.name)}** isimli seçenek ayar rolü ${rol} olarak tanımladı.`)]})
