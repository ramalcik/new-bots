...
const Private = require('../../../../_SYSTEM/Databases/Schemas/Plugins/Guild.Private.Rooms.js');
const {VoiceChannels, TextChannels, CategoryChannels, Roles} = require("../../../../_SYSTEM/Databases/Schemas/Guards/Backup/Guild.Sync");
  ...
  onLoad: function (client) {
    client.on('interactionCreate', async i => {
      ...
      let Row = new MessageActionRow().addComponents(
        ...
        .setCustomId("munuryöneticipaneli")
      ...
      let RowTwo = new MessageActionRow().addComponents(
        ...
        let cartelim = i.guild.members.cache.get(i.user.id)
        ayarlar = Data.Ayarlar
        if (i.customId === 'munuryöneticipaneli') {
          if(i.values[0] == "sunucuduzenle") {
            ...
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            showModal(modal, {
              client: client,
              interaction: i 
            })
          ...
          if(i.values[0] == "rolsüzver") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            let rolsuzcartelim =  i.guild.members.cache.filter(m => m.roles.cache.filter(r => r.id !== i.guildId).size == 0);
            rolsuzcartelim.forEach(roluolmayanlar => { 
                roller.kayıtsızRolleri.some(x => roluolmayanlar.roles.add(x).catch(err => {})) 
            });
            await i.reply({content: `Başarıyla sunucuda rolü olmayan **${rolsuzcartelim.size}** üyeye kayıtsız rolü verilmeye başlandı! ${i.guild.emojiGöster(emojiler.onay_munur)}`, ephemeral: true})
          }
          if(i.values[0] == "tagsızver") {
            ...
            let tagsizcartelim = guild.members.cache.filter(m => m.user.username.includes(ayarlar.tag) && !m.roles.cache.has(roller.tagRolü) && !m.roles.cache.has(roller.şüpheliRolü) && !m.roles.cache.has(roller.yasaklıTagRolü) &&  !m.roles.cache.has(roller.jailRolü) && !roller.kayıtsızRolleri.some(x => m.roles.cache.has(x)))
            tagsizcartelim.forEach(roluolmayanlar => { 
            ...
            await i.reply({content: `Başarıyla taglı olup rolü olmayan **${tagsizcartelim.size}** üyeye taglı rolü verildi, ve tagsız **${rollütagsiz.size}** üyeden geri alınmaya başlandı! ${i.guild.emojiGöster(emojiler.onay_munur)}`, ephemeral: true})
          } 
          if(i.values[0] == "etkinlikçekilişdağıt") {
            ...
            //siktirgit oç
            await i.reply({content: `Başarıyla **${olmayanlar.size}** üyeye **Etkinlik & Çekiliş** katılımcısı dağıtılmaya başlandı! ${i.guild.emojiGöster(emojiler.onay_munur)} `, ephemeral: true})
          }
          if(i.values[0] == "özelkarakter") {
            ...
            ünlemliler.forEach(async (cartelim) => {
              await cartelim.setNickname(cartelim.displayName.replace("!","").replace("!!","").replace("!!!","").replace("-","").replace("+","").replace("'","").replace("^", "")).catch(err => {})
            })
            await i.reply({content: `Başarıyla **${ünlemliler.size}** üyenin üzerindeki \`Boşluk, Ünlem vs..\` temizlenmeye başlandı! ${i.guild.emojiGöster(emojiler.onay_munur)} `, ephemeral: true})
          }
          if(i.values[0] == "syncpublic") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            i.reply({content: `${i.guild.emojiGöster(emojiler.onay_munur)} Başarıyla tüm public(**${i.guild.channels.cache.filter(x => x.parentId && x.parentId == kanallar.publicKategorisi && x.type == "GUILD_VOICE").size}** kanal) ses kanalları senkronize olmaya başladı. Bu işlem biraz uzun sürebilir...`, ephemeral: true})
          ...
          if(i.values[0] == "syncsç") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            i.reply({content: `${i.guild.emojiGöster(emojiler.onay_munur)} Başarıyla tüm sorun çözme(**${i.guild.channels.cache.filter(x => x.parentId && x.parentId == kanallar.sorunCozmeKategorisi && x.type == "GUILD_VOICE").size}** kanal) ses kanalları senkronize olmaya başladı. Bu işlem biraz uzun sürebilir...`, ephemeral: true})
          ...
          if(i.values[0] == "syncregister") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            i.reply({content: `${i.guild.emojiGöster(emojiler.onay_munur)} Başarıyla tüm teyit(**${i.guild.channels.cache.filter(x => x.parentId && x.parentId == kanallar.registerKategorisi && x.type == "GUILD_VOICE").size}** kanal) ses kanalları senkronize olmaya başladı. Bu işlem biraz uzun sürebilir...`, ephemeral: true})
          ...
          if(i.values[0] == "syncstreamer") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            i.reply({content: `${i.guild.emojiGöster(emojiler.onay_munur)} Başarıyla tüm yayıncı (**${i.guild.channels.cache.filter(x => x.parentId && x.parentId == kanallar.streamerKategorisi && x.type == "GUILD_VOICE").size}** kanal) ses kanalları senkronize olmaya başladı. Bu işlem biraz uzun sürebilir...`, ephemeral: true})
          ...
          if(i.values[0] == "syncguild") {
            if((roller.kurucuRolleri && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku))) && !cartelim.permissions.has('ADMINISTRATOR')) return await i.reply({content: `${cevaplar.prefix} Yeterli yetkiye sahip değilsin.`, ephemeral: true})
            i.reply({content: `${i.guild.emojiGöster(emojiler.onay_munur)} Başarıyla tüm sunucu ses kanalları (**${i.guild.channels.cache.filter(x => x.parentId && x.type == "GUILD_VOICE").size}** kanal)senkronize olmaya başladı. Bu işlem biraz uzun sürebilir...`, ephemeral: true})
...
client.on('modalSubmit', async (modal) => {
  if(modal.customId == "limitOzelOdacik") {
    ...
    let Data = await Private.findOne({guildID: guild.id, userID: cartelim.id})
    if(!Data) {
      await modal.deferReply({ ephemeral: true })
  ...
  if(modal.customId == "ozelOdaBanla") {
    ...
    if(sesKanalı) { 
      ...
      if(izinVerilcek) {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla "${sesKanalı}" kanalında ${izinVerilcek} üyesi yasaklandı. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      } else {
        await modal.deferReply({ ephemeral: true })
  ...
  if(modal.customId == "ozelOdaIzin") {
    ...
    if(sesKanalı) { 
      ...
      if(izinVerilcek) {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla "${sesKanalı}" kanalına ${izinVerilcek} üyesi eklendi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      } else {
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Belirttiğiniz ID ile bir üye eşleşmedi. Lütfen geçerli bir ID numarası girin. ${cevaplar.prefix}` , ephemeral: true })
      }
  ...
  if(modal.customId == "ozelOdaOlusturma") {
    ...
    guild.channels.create(`${odaIzin != "HAYIR"  ? "🔓" : "🔒"} ${odaIsmi}`, {
      ...
      if(odaIzin == "HAYIR") { 
        await kanal.permissionOverwrites.edit(cartelim.guild.roles.everyone.id, { CONNECT: false,SPEAK: true, STREAM: true });
      } else { 
        await kanal.permissionOverwrites.edit(cartelim.guild.roles.everyone.id, { CONNECT: true, SPEAK: true, STREAM: true }); 
      }
      setTimeout(async () => {
        if(kanal && kanal.members.size <= 0) {
          setTimeout(async () => {
            await Private.deleteOne({guildID: guild.id, userID: cartelim.id})
      ...
      if(kanal) kanal.send({content : `Özel Oda Yönetim Paneline Hoş Geldin! ${cartelim}
...
Özel odanız sadece izinliler olarak ayarlandığında izin verdiğiniz herkes giriş yapabilir.`, components: [Row, RowTwo]})
      await modal.deferReply({ ephemeral: true })
      await Private.updateOne({guildID: guild.id, userID: cartelim.id}, {$set: {"Date": Date.now(), "voiceChannelId": kanal.id, "messageChannelId": kanal.id}}, {upsert: true});
      await Private.updateOne({guildID: guild.id, userID: cartelim.id}, {$set: {"Date": Date.now(), "voiceChannelId": kanal.id, "messageChannelId": kanal.id}}, {upsert: true})
  ...
  if(i.customId == "herkeseAcik_ozelOda") {
    ...
    if(sesKanalı) { 
      ...
      let RowTwo = new MessageActionRow().addComponents(
        new MessageButton()
        .setCustomId("herkeseAcik_ozelOda")
        .setLabel(sesKanalı.permissionsFor(cartelim.guild.roles.everyone).has('CONNECT') ? "Sadece İzinliler'e Ayarla" : "Herkese Açık Ayarla")
        .setStyle(sesKanalı.permissionsFor(cartelim.guild.roles.everyone).has('CONNECT') ? "SECONDARY" : "PRIMARY"),
        new MessageButton()
        .setCustomId("odaIzinSıfırla")
        .setLabel("Kanal İzinleri Temizle")
        .setStyle("DANGER"),
      ...
      if (sesKanalı.permissionsFor(cartelim.guild.roles.everyone).has('CONNECT')) {
        await sesKanalı.permissionOverwrites.edit(cartelim.guild.roles.everyone.id, { CONNECT: false, SPEAK: true, STREAM: true });
        sesKanalı.setName(sesKanalı.name.replace("🔓", "🔒"))
        RowTwo.components[0].setStyle("PRIMARY").setLabel(`Herkese Açık Ayarla`)
        i.update({components: [Row, RowTwo]})
      } else {
        await sesKanalı.permissionOverwrites.edit(cartelim.guild.roles.everyone.id, { CONNECT: true, SPEAK: true, STREAM: true });
...
5 Saniye içerisinde ${sesKanalı} kanalınız silinecektir. ${cartelim.guild.emojiGöster(emojiler.onay_munur)}`, ephemeral: true})
    } else {
      return i.reply({content: `Sistemsel bir hata oluştu, lütfen yöneticilere başvurun.`, ephemeral: true});
    }
  }
  if(i.customId == "kanalBilgisi_ozelOda") {
    ...
    if(sesKanalı) {
      ...
      i.reply({content: `
Ses kanalın görünürlüğü: **${sesKanalı.permissionsFor(cartelim.guild.roles.everyone).has('CONNECT') ? "Herkese Açık!" : "Sadece İzinliler!"}**
Oluşturulma tarihi: <t:${String(Data.Date).slice(0, 10)}:F> (<t:${String(Data.Date).slice(0, 10)}:R>)

Ses kanalında izinliler:
${izinliler.length > 0 ? izinliler.map(x => `> ${cartelim.guild.members.cache.get(x)} (\`${x}\`)`).join("\n") : "İzinli bulunamadı!"}

Ses kanalında yasaklılar:
${yasaklılar.length > 0 ? yasaklılar.map(x => `> ${cartelim.guild.members.cache.get(x)} (\`${x}\`)`).join("\n") : "Yasaklı bulunamadı!"}
...
`, ephemeral: true})
  ...
  .addComponents(
    ...
    .setMinLength(3)
    .setMaxLength(60)
    .setPlaceholder(`Örn: munur'ın Odası`)
    .setRequired(true),
    new TextInputComponent()
