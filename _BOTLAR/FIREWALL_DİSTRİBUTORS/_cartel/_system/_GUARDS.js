...
module.exports = {
    Isim: "güvenlik",
    Komut: ["ANAN", "guard-settings","guards","guard"],
    Kullanim: "",
    Aciklama: "",
    Kategori: "-",
    Extend: true,
  ...
  onRequest: async function (client, message, args) {
    ...
    let anaMenu = new cartelinEmbedi().setColor("RANDOM")
...
\` • \` Numaralar: \` ${ayarlar.Numaralar ? ayarlar.Numaralar.map(x => "+1" + x).join(", ")  : "Numara Bulunamadı!"} \``: ""}
        ...
        message.channel.send({embeds: [anaMenu], components: [oneRows]}).then(async (msg) => {
            ...
            collector.on('collect', async (i) => {
              ...
              if(i.customId == "guard_manager") {
                ...
                msg.delete().catch(err => {})
                message.channel.send({embeds: [anaMenu], components: [münürRow, backMenu]}).then(async (gmsg) => {
                    var filter = (i) => i.user.id == message.member.id
                    ...
                    collector.on('collect', async (i) => {
                        ...
                        if(i.customId == "limitSettings") {
                            ...
                            message.channel.send({embeds: [new cartelinEmbedi().setColor("RANDOM")
...
Olarak ayarlanmış fakat işlem sayısı ayarlanmamış. Ayarlamak için lütfen kaç işlem olacağını belirtin.`).setFooter(`Ana menüye dönmek için "iptal" yazabilirsiniz.`)]}).then(async (yarramm) => {
    ...
    collector.on('collect', async (m) => {
        ...
        let guardSettings = await GUARD_SETTINGS.findOne({guildID: sistem.SUNUCU.GUILD})
        yarramm.edit({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla sunucunun limit ayarları güncellendi.
...
\` • \` SMS/MMS Bilgilendirme: \` ${sistem.SMS ? "Aktif ✅" : "Devre-Dışı ❌"} \`
\` • \` Whatsapp Bilgilendirme: \` ${ayarlar.WHATSAPP ? "Aktif ✅" : "Devre-Dışı ❌"} \`
\` • \` Telegram Bilgilendirme: \` ${ayarlar.TELEGRAM ? "Aktif ✅" : "Devre-Dışı ❌"} \`
\` • \` Numaralar: \` ${ayarlar.Numaralar ? ayarlar.Numaralar.map(x => "+1" + x).join(", ")  : "Numara Bulunamadı!"} \``: ""}
...
${guardSettings.Process ? guardSettings.Process.length > 0 ? guardSettings.Process.sort((a, b) => b.date - a.date).slice(0, 10).map((x, index) => `\` ${index + 1} \` <@!${x.target}> [**${x.type}**] ${x.member ? `(<@!${x.member}>) ` : ""}(<t:${String(x.date).slice(0, 10)}:R>)` ).join("\n")  : "Yakın zamanda bir kayıt bulunamadı." : "Yakın zamanda bir kayıt bulunamadı."}`)
              ...
              if(i.customId == "guard_manage") {
                ...
                let guildSettings = await GUILD_SETTINGS.findOne({guildID: sistem.SUNUCU.GUILD})
                if(guildSettings) ayarlar = guildSettings.Ayarlar
                let listEmbed = new cartelinEmbedi().setColor("RANDOM").setDescription(`Aşağı da \`${ayarlar ? ayarlar.serverName ? ayarlar.serverName : message.guild.name : message.guild.name }\` sunucusunun beyaz listede ki üyeleri ve rolleri listelenmektedir. Bu listeyi sadece sunucu sahibi güncelleyebilir.
...
.addField("Chat Beyaz Liste", `${ayarlar.chatİzinliler.length >= 1 ? ayarlar.chatİzinliler.map((x, index) => `\` ${index + 1} \` ${message.guild.roles.cache.get(x) || message.guild.members.cache.get(x) || message.guild.channels.cache.get(x)  || x}`).join("\n") : 'Listede sonuç bulunamadı!'}`, false)
        ...
        message.channel.send({embeds: [new cartelinEmbedi().setDescription(`**Merhaba!** ${message.member.user.tag}

${cartelim} isimli ${message.guild.roles.cache.get(cartelim.id) ? "rolü" : message.guild.channels.cache.get(cartelim.id) ? "kanalı" : "üyeyi"} hangi güvenliğe eklemek/çıkarmak istersiniz?

**Bilgilendirme**:
Belirtilen üye, rol veya kanal ekli bir beyaz listede bulunuyorsa düğmenin rengi yeşile dönmektedir, bulunmuyor ise düğme füme rengine dönmektedir. :tada:`).setColor("RANDOM")], components: [whiteListRow, whiteListRowTwo, whiteListRowThree]}).then(async (msg) => {
            var filter = (i) => i.user.id == message.member.id
