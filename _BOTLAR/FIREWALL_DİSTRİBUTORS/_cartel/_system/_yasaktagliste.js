  ...
  onRequest: async function (client, message, args) {
      let embed = new cartelinEmbedi()
        ...
        if (["ekle", "Ekle", "add", "Add","at"].some(kontrol => kontrol === args[0])) {
			let tags = args[1];
            if(!tags) return message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined); 
			if (ayar.yasakTaglar && ayar.yasakTaglar.includes(tags)) return message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.no_munur)} Eklemeye çalıştığınız \`${tags}\` zaten yasaklı tag/etiket listesinde.`)]}).then(x => {
                setTimeout(() => {
                    x.delete()
                }, 7500);
            ...
            GUILDS_SETTINGS.findOne({guildID: sistem.SUNUCU.GUILD}, async (err, res) => {
                ...
                if(ayar.yasakTaglar) yasakTags = [...ayar.yasakTaglar]
					...
					await message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarılı bir şekilde \`${tags}\` tagını yasaklı taglar/etiketler listesine ekledin.
...
\`\`\``).catch(err => {})
        ...
        if (["kaldır", "sil", "remove", "delete", "Sil","çıkar","Çıkart"].some(kontrol => kontrol === args[0])) {
            let tags = args[1];
            if(!tags) return message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined);
            if (ayar.yasakTaglar && !ayar.yasakTaglar.includes(tags)) return message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.no_munur)} Eklemeye çalıştığınız \`${tags}\` zaten yasaklı tag/etiket listesinde bulunmuyor.`)]}).then(x => {
                setTimeout(() => {
                    x.delete()
                }, 7500);
            }),message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
            const findUser = ayar.yasakTaglar.find(munur => munur == tags);
            await GUILDS_SETTINGS.updateOne({ guildID: message.guild.id }, { $pull: { "Ayarlar.yasakTaglar": findUser } }, { upsert: true })
            let cartelimler = message.guild.members.cache.filter(u => u.user.username.includes(tags) || u.user.discriminator.includes(tags))
            await message.channel.send({embeds: [embed.setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Başarılı bir şekilde \`${tags}\` tagını yasaklı taglar/etiketler listesinden çıkarttınız.
${message.guild.emojiGöster(emojiler.Terfi.miniicon)} Bu tagda veya etikette bulunan üyeler: ${cartelimler.map(x => x).slice(0,7).join(", ")} ${cartelimler.size > 7 ? `ve ${cartelimler.size - 7} daha fazlası...` : ''}`)]}).then(x => {
setTimeout(() => {
x.delete()
}, 7500);
...
Üzerinizde bulunan **\` ${args[1]} \`** bu sembol veya etiket yasağı kaldırıldığından dolayı sizi yasaklı kategorisinden çıkarttık.`).catch(err => {})
    ...
    if(!args[0]) {
        message.reply({embeds: [embed.setAuthor(null).setFooter(`${sistem.botSettings.Prefixs[0]}yasaktag <[ekle/kaldır]> <[tag]>`).setDescription(`**Merhaba!** ${message.member.user.tag}
