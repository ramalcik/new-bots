            ...
            collector.on('collect', async (i) => {
                if(i.customId == `${i.user.id}+denetim-roller`) {
                    if(i.values[0] != "0") {
                        ...
                        await i.update({components: [], content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla rol kurma komutlarına istek gönderildi.`})
            ...
            collector.on('collect', async (i) => {
                if(i.customId == `${i.user.id}+denetim-kanallar`) {
                    if(i.values[0] != "0") {
                        ...
                        await i.update({components: [], content: `${message.guild.emojiGöster(emojiler.onay_munur)} Başarıyla kanal kurma komutlarına istek gönderildi.`})
                        message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined).catch(err => {})
                    } else {
                        i.reply({content: `Son 1 saat içerisinde silinen bir veri bulunamadı.`, ephemeral: true})
                    }
