...
const TalentPerms = require('../../../../_SYSTEM/Databases/Schemas/Global.Guild.Settings');
const task = require('../../../../_SYSTEM/Databases/Schemas/Plugins/Client.Users.Tasks');
  ...
  onLoad: function (client) {
    client.on('modalSubmit', async (modal) => {
      ...
      if(modal.customId == "tp-kaldır") {
        ...
        let cmd = check.talentPerms.find(munur => munur.Commands == cmdName)
        if(!cmd) {
          await modal.deferReply({ ephemeral: true })
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> kaldırıldı. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      ...
      if(modal.customId == "tp-detay") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({embeds: [new cartelinEmbedi().setThumbnail(guild.iconURL({dynamic: true})).setFooter(cartelim.user.tag, cartelim.user.avatarURL({dynamic: true})).setDescription(`Aşağı da **${cmdName}** isimli rol (ver/al) veya alt komutun detaylı bilgileri belirtilmiştir.\n
...
${cmd.Permission ? cmd.Permission.map(x => guild.roles.cache.get(x)).join(", ") : "@rol bulanamadı"} rol veya rolleri kullanabilir.`)] , ephemeral: true })
      ...
      if(modal.customId == "tp-ekle") {
        ...
        let check = await TalentPerms.findOne({guildID: guild.id})
        let cmd = check.talentPerms ? check.talentPerms.find(munur => munur.Commands == cmdName) : undefined
        if(cmd) {
          await modal.deferReply({ ephemeral: true })
          return await modal.followUp({content: `Belirtilen isimde aktif bir komut bulunmakta. ${cevaplar.prefix}` , ephemeral: true })
        }
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `Başarıyla ${cmdName} komutu <t:${String(Date.now()).slice(0, 10)}:R> eklendi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
  ...
  onRequest: async function (client, message, args) {
    if(!sistem._rooter.rooters.includes(message.member.id) && message.guild.ownerId != message.member.id) return;  
...
${komutlar.map(x => `${message.guild.emojiGöster("munur_arrow")} **${x.name}** (${x.roles})`).join("\n")}` : ``}`)
      ...
      collector.on('end', (collected, reason) => {
          if(reason == "time") {
            ...
            load.edit({components: [Row], embeds: [
              new cartelinEmbedi().setDescription(`Zaman aşımına uğradığı için işleminiz sonlandırıldı. ${cevaplar.prefix}`)
            ]});
            setTimeout(() => {
