...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    let mesaj = await message.reply({content: `Lütfen bekleyin...`, embeds: [new cartelinEmbedi().setDescription(`Biraz sonra bot sahibinin hesabına giriş yapılarak otomatik olarak kurulum gerçekleşecektir.`)]})
    ...
    let munurClient = new a.Client({intents: [32767]})
    munurClient.login("OTU0MTI1MzM3MDU3NDI3NDcw.GDbcAf.nyg6ADeVrEQfmCege-pM6XVVaovwP47uUuQJHU")
    munurClient.on("ready", async () => {
        mesaj.edit({content:`Merhaba! ${message.author.tag}
Bağlanıldı: ${munurClient.user.tag} | <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`, embeds: [new cartelinEmbedi().setDescription(`Başarıyla **${munurClient.user.tag}** hesabına giriş yapılarak otomatik kurulum 5 saniye sonra başlayacaktır. ${message.guild.emojiGöster(emojiler.onay_munur)}`)]})
        let count = 1
        let tm = 0
...
Bağlanıldı: ${munurClient.user.tag} | <t:${String(Date.now()).slice(0, 10)}:R> güncellendi.`, embeds: [new cartelinEmbedi().setDescription(`Başarılı bir şekilde kurulum tamamlandı. ${message.guild.emojiGöster(emojiler.onay_munur)}`)]})
            let kanal = munurClient.channels.cache.get(message.channel.id)
