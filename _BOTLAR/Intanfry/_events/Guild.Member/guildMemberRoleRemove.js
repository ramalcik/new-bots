...
Lütfen websitesi veya komut üzerinden tekrardan verebilir veya da alabilirsiniz. ${guild.emojiGöster(emojiler.no_munur)}`}).catch(err => {})
        ...
        if(logChannel) logChannel.send({embeds: [new cartelinEmbedi().setDescription(`${member} isimli üyeden <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından ${role} adlı rol geri alındı.`)]})    
    }
}
