...
module.exports = async (member, role) => {
        ...
        _staffs = reload('../../../../_SYSTEM/Additions/Staff/Sources/_settings.js');
...
Lütfen websitesi veya komut üzerinden tekrardan verebilir veya da alabilirsiniz. ${guild.emojiGöster(emojiler.no_munur)}`}).catch(err => {})
    } else {
        ...
        if(roller && roller.yasaklıRoller && entry && entry.executor && roller.yasaklıRoller.some(x => role.id == x) && !sistem._rooter.rooters.includes(entry.executor.id)) {
        ...
        if(logChannel) logChannel.send({embeds: [new cartelinEmbedi().setDescription(`${member} isimli üyeye <t:${String(Date.now()).slice(0, 10)}:R> ${entry.executor} tarafından ${role} adlı rol verildi.`)]})      
    }
}
