...
const VMutes = require('../../../../_SYSTEM/Databases/Schemas/Punitives.Vmutes');
...
const Welcome = require('../../../../_SYSTEM/Databases/Schemas/Others/Guild.Welcome.Settings');
...
module.exports = async (member) => {
    ...
    if(amkSürekliÇıkıyoGiriyo >= 3) {
      ...
      if(hgKanalı) hgKanalı.send({content: `${member} isimli üye birden fazla **Giriş-Çıkış** işlemi yaptığından dolayı sunucudan uzaklaştırıldı. ${member.guild.emojiGöster(emojiler.no_munur)}`}).then(x => {})
    ...
    if(OneWeak) {
        ...
        return member.guild.kanalBul("şüpheli-log").send({embeds: [new cartelinEmbedi().setDescription(`${member} isimli üye sunucuya katıldı fakat hesabı ${global.timeTag(Date.parse(member.user.createdAt))} açıldığı için şüpheli olarak işaretlendi.`)]});
    };
...
\`\`\``).catch(err => {})
        await member.guild.kanalBul("yasaklı-tag-log").send({embeds: [new cartelinEmbedi().setDescription(`${member} isimli üye sunucuya katıldı fakat isminde yasaklı tag/etiket barındırdığından dolayı yasaklı olarak işaretlendi.`)]});
    ...
    if(cezaPuan >= 50) {
    	await member.setRoles(_set.şüpheliRolü)
	    await member.send({embeds: [new cartelinEmbedi().setDescription(`${member.guild.emojiGöster(emojiler.Cezalandırıldı)} ${member} Ceza puanın \`${cezaPuan}\` olduğu için otomatik olarak şüpheli hesap olarak belirlendin.`)]}).catch(x => {});
        ...
        if(Data) {
          ...
          if(Data.Webhook) {
              ...
              hoşgeldinKanal.send({content: `${hoşgeldinTextiAmınakodum}`});
                 ...
                 hoşgeldinKanal.send({embeds: [new cartelinEmbedi().setDescription(`${member}, ${_set.serverName ? `${_set.serverName}` : member.guild.name} Sunucumuza Hoş Geldin.
...
${member.guild.channels.cache.filter(x => x.parentId == kanallar.registerKategorisi && x.type == "GUILD_VOICE").random()} Kanalına katılarak "${_set.isimyas ? `İsim | Yaş` : `Nick`}" vererek kayıt olabilirsiniz.
...
async function rolTanımlama(üye, rol) {
    ...
    await üye.roles.set(startRoles).then(async (munur) => {})
}
