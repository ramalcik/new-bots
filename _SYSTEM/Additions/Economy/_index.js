...
class Coin {
    ...
    static async updateBalance(id, balance = "10", process = "add", type = 1) {
        ...
        if(!ramalcim) return 0;
        let account = await Users.findOne({_id: ramalcim.id})
        if(!account) return 0;
        ...
        switch (type) {
            case 0: {
                if(process == "remove") {
                    if(account && gold-1 >= 0) return await Users.updateOne({_id: ramalcim.id}, {$inc: {"Gold": -balance}}, {upsert: true});
                } else if(process == "add") return await Users.updateOne({_id: ramalcim.id}, {$inc: {"Gold": balance}}, {upsert: true})
            };
            case 1: {
                if(process == "remove") {
                    if(account && coin-1 >= 0) return Users.updateOne({_id: ramalcim.id}, {$inc: {"Coin": -balance}}, {upsert: true});
                } else if(process == "add") return await Users.updateOne({_id: ramalcim.id}, {$inc: {"Coin": balance}}, {upsert: true})
            };
