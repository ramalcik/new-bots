...
module.exports = async (options) => {
	...
	if (data.has(options.message.author.id) || data.has(options.opponent.id)) {
		return options.message.react(options.message.guild.emojiGöster(emojiler.no_munur)).catch(err => {});
	}
	...
	if (oppenent.bot || oppenent.id === challenger.id) return options.message.react(options.message.guild.emojiGöster(emojiler.no_munur)).catch(err => {});
	let acceptbutton = new Discord.MessageButton()
		.setStyle('SECONDARY')
		.setLabel(options.buttons.accept)
		.setCustomId("accept")
.setEmoji(options.message.guild.emojiGöster(emojiler.onay_munur));;
	let denybutton = new Discord.MessageButton()
		.setStyle('DANGER')
		.setLabel(options.buttons.deny)
		.setCustomId("deny")
.setEmoji(options.message.guild.emojiGöster(emojiler.no_munur));;
...
.setEmoji(options.message.guild.emojiGöster(emojiler.no_munur));;
			...
			let row = new Discord.MessageActionRow()
				...
				.addComponents(btn2)
				.addComponents(btn3);
			const _embed = new cartelinEmbedi()
					...
					if (!checkHealth(player)) {
						...
						if (msg.customId === id1) {
							...
							if (gameData[player].member.id == options.message.author.id) {
								const __embed = new cartelinEmbedi()
