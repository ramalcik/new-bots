...
GuildMember.prototype.Register = async function (name, gender = undefined, registrant = undefined) {
    ...
    let seskanal = registrant.guild.channels.cache.filter(x => x.parentId == kanallar.publicKategorisi && x.type == "GUILD_VOICE" && x.id != kanallar.sleepRoom && (_statSystem.musicRooms && _statSystem.musicRooms.some(kanal => x.id != kanal))).random()
...
Sana İyi Sohbetler Dilerim! :tada: :tada: :tada:`}).catch(err => {})
    ...
    if(kayıtLog) kayıtLog.send({embeds: [new cartelinEmbedi().setDescription(`${this} isimli üye ${registrant} yetkili tarafından <t:${String(Date.now()).slice(0, 10)}:R> **${gender}** olarak \`${this.guild.name}\` sunucusuna kayıt edildi.`)]})
...
${this} (**${this.user.tag}**)
ID: ${this.id}`, embeds: [new cartelinEmbedi()
                ...
                .setFooter("bu bilgilendirmeye mesajına uyarak, lütfen eski yetkilini sunucuya tekrardan davet et.")]}).catch(err => { })
              const findUser = Yetkili.Staffs.find(munur => munur.id == this.id);
        ...
        if(!manuel) {
            ...
            let embeds = new cartelinEmbedi()
             ...
             if(Data) {
                ...
                embeds.addField(`Detaylar`,
                    `
Yetkiye Başlatan: ${userData.StaffGiveAdmin ? sorumlusu ? `${sorumlusu} (\`${sorumlusu.displayName}\`)` : `<@${userData.StaffGiveAdmin}> (\`Sunucudan Ayrılmış!\`)`  : `\` Tespit Edilemedi! \`` }
Yetkide Kalma Süresi: \` ${Data.Baslama ? sureCevir(Date.now() - Date.parse(Data.Baslama)) : `Tespit Edilemedi!`} \` 
Yetkiye Başlama Tarihi: ${Data.Baslama ? ` <t:${String(Date.parse(Data.Baslama)).slice(0, 10)}:R> (<t:${String(Date.parse(Data.Baslama)).slice(0, 10)}:F>)` : `\` Tespit Edilemedi! \``}
             ...
             embeds.addField("Son rolleri şunlardır", `${arr.map(x => this.guild.roles.cache.get(x)).join(", ")}`)
...
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].filter(x => this.guild.roles.cache.get(x)).map(x => this.guild.roles.cache.get(x))}`, embeds: [
    ...
    collector.on('collect', async (i) => {
        ...
        if(kapan) {
            ...
            Row.components[0].setLabel(`${kapan.user.tag}`)
...
O zaman bana ${kapan} (${kapan.user.tag} | ${kapan.id}) yazarsanız derdinize deva olabiliriz.`}).then(a => {
    ...
    if(userData && userData.Tagged && userData.TaggedGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.TaggedGiveAdmin }) 
           if(Yetkili && Yetkili.Taggeds) {
                ...
                const findUser = Yetkili.Taggeds.find(munur => munur.id == this.id);
    ...
    if(userData && userData.Name && userData.Registrant) {
        let Yetkili = await Users.findOne({_id: userData.Registrant }) 
          if(Yetkili && Yetkili.Records) {
              ...
              if(koşAnanıSikiyorlar) koşAnanıSikiyorlar.send({content: `Kayıt ettiğiniz ${this} (${this.user.tag}) (**Ceza Puanı**: \`${await this.cezaPuan()}\`) üyesi sunucuda **Kural Dışı** eylem sergiledi ve cezalandırıldı.
...
GuildMember.prototype._views = async function () {
    return new Promise(async (resolve, reject) => {
            ...
            resolve(`Başarıyla ${this.user.tag} üyesinin görüntülemesi ${_cached.Views || 0} oldu.`)
    })
}
  ...
  GuildMember.prototype.Delete = async function (type = "default") {
    ...
    if(userData && userData.Name && userData.Registrant) {
        let Yetkili = await Users.findOne({_id: userData.Registrant }) || {}
          if(Yetkili && Yetkili.Records) {
              const koşAnanıSikiyorlar = this.guild.members.cache.get(userData.Registrant)
              const findUser = Yetkili.Records.find(munur => munur.User == this.id);
    ...
    if(userData && userData.Staff && userData.StaffGiveAdmin) {
        let Yetkili = await Users.findOne({_id: userData.StaffGiveAdmin }) || {}
          if(Yetkili && Yetkili.Staffs) {
              ...
              if(BabanıSikeyim) BabanıSikeyim.send({embeds: [new cartelinEmbedi().setDescription(`${BabanıSikeyim} Merhaba, ${this} (\`${this.id}\`) isimli çektiğin yetkili yetkiyi sunucudan çıkarak <t:${String(Date.now()).slice(0, 10)}:R> bıraktı.`).setFooter("bu bilgilendirmeye mesajına uyarak, lütfen eski yetkilini sunucuya tekrardan davet et.")]}).catch(err => { })
              const findUser = Yetkili.Staffs.find(munur => munur.id == this.id);
              await Users.updateOne({ _id: userData.StaffGiveAdmin }, { $pull: { "Staffs": findUser } }, { upsert: true })
              client.Upstaffs.removePoint(userData.StaffGiveAdmin, -_statSystem.points.staff, "Yetkili")
          } 
      ...
      if(userData && userData.Staff) {
        ...
        let yetkiyiBırakan = this.guild.kanalBul("yetki-bırakan")
        if(yetkiyiBırakan) yetkiyiBırakan.send({content: `${this} [**\` ${this.id} \`**]
${[...roller.altYönetimRolleri, ...roller.yönetimRolleri, ...roller.üstYönetimRolleri].map(x => this.guild.roles.cache.get(x))}`,embeds: [new cartelinEmbedi().setFooter("bu bilgilendirme mesajına uyarak, lütfen eski yetkilimizi sunucumuza davet edin.").setDescription(`${this} isimli eski yetkili, yetkiyi sunucudan ayrılarak **<t:${String(Date.now()).slice(0, 10)}:R>** yetkisini saldı.`).addField("Son rolleri şunlardır", `${arr.map(x => this.guild.roles.cache.get(x)).join(", ")}`)]}) 
    }
}
...
GuildMember.prototype.Leaders = async function(text, point, option = {
    ...
    if(roles && roles.length > 0 && roles.some(b => this.roles.cache.has(b.role))) {
        ...
        for (let index = 0; index < roles.length; index++) {
            ...
            .setAuthor(`${this.displayName} ${header} Puan'ı Kazandı!`, this.user.displayAvatarURL({dynamic: true}))
            .setDescription(`
...
Yapılan İşlem Türü İse "**${detay.type}**" Olarak Tespit Edildi.`).setFooter(this.guild.name, this.guild.iconURL({dynamic: true}))
            if(option.user)  {
                let target = this.guild.members.cache.get(option.user)
                embed.addField(`İşlem Yapılan Kullanıcı`, `${ target ? `${target} (\`${target.id}\`)` : `<@${option.user}> (\`Sunucudan Ayrılmış\`)`}`)
            }
            if(option.channel)  {
                let target = this.guild.channels.cache.get(option.channel) || this.guild.channels.cache.get(option.channel.id)
                embed.addField(`İşlem Yapılan Kanal`, `${target ? `${target}` : `@deleted-channel`}`)
            }
            if(option.role)  {
                let target = this.guild.roles.cache.get(option.role)
                embed.addField(`İşlem Yapılan Rol`, `${target ? `${target}` : `@deleted-roles`}`)
            }
...
GuildMember.prototype.addPunitives = async function(type, staff, reason = "Sebep belirtilmedi.", message, duration, muteFlood, rulesFlood, nofi = 1) {
    ...
    cezano = cezano == 0 ? 1 : cezano + 1;
        ...
        if(nofi == 0 && !muteFlood && !rulesFlood) await message.editReply({embeds: [new cartelinEmbedi().setDescription(`${this.toString()} isimli üyeye **${reason}** sebebiyle ${duration ? `\`${moment.duration(ms(duration)).format('Y [Yıl,] M [Ay,] d [Gün,] h [Saat,] m [Dakika]')}\` boyunca` : ``} ${type == "Kalkmaz Yasaklama" ? 
        "tamamiyle kalkmaz yasaklamaya tabi tutuldun."
    : type == "Yasaklama" ? 
        "yasakladın."
    : type == "Cezalandırılma" ? 
    ...
    : type == "Uyarılma" ?
    "uyardın."
    : `"${type}" türünde ceza-i işlem uygulandı.`} ${message.guild.emojiGöster(emojiler.onay_munur)}`).setFooter(`${this.user.tag} • Ceza Numarası: #${cezano} • ${type}`, this.user.avatarURL({dynamic: true}))], components: []})
        switch(type) {
