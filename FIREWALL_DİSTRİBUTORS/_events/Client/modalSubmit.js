...
module.exports = async (modal) => {
    ...
    if(modal.customId == "isimDüzenleme") {
        ...
        if(ayarlar.type && ayarlar.isimyas) {
            if(roller.Yetkiler.some(x => cartelim.roles.cache.has(x)) && !roller.kurucuRolleri.some(oku => cartelim.roles.cache.has(oku)) && !cartelim.permissions.has('ADMINISTRATOR')) {
    ...
    if(modal.customId == "sunucuDüzenleme") {
        ...
        if(gerçekleşenİşlemler.length > 0) {
            ...
            if(logKanal) logKanal.send({embeds: [new cartelinEmbedi().setColor("ORANGE")
    .setFooter(cartelim.user.tag + " tarafından güncellendi.",cartelim.user.avatarURL({dynamic: true}))
          .setDescription(`${guild.name} sunucusunda güncelleme ${cartelim} tarafından yapıldı.
...
\`\`\``)]})
            await modal.deferReply({ ephemeral: true })
            return await modal.followUp({embeds: [new cartelinEmbedi().setColor("RANDOM").setDescription(`Başarıyla ${guild.name} sunucusu üzerinde işlem(ler) <t:${String(Date.now()).slice(0, 10)}:F> tarihinde gerçekleştirildi/güncelleştirildi.
