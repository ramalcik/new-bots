...
module.exports = {
  ...
  onRequest: async function (client, message, args) {
    ...
    CategoryChannels.findOne({ channelID: args[0] }, async (err, data) => {
      if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen kategori kanalı geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
...
async function voiceKur(idcik, message, embed, parentID) {
    VoiceChannels.findOne({ channelID: idcik }, async (err, data) => {
        if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen ses kanal geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
        const newChannel = await message.guild.channels.create(data.name, {
          type: 'GUILD_VOICE',
...
async function textKur(idcik, message, embed, parentID) {
    TextChannels.findOne({ channelID: idcik }, async (err, data) => {
        if (!data) return message.channel.send(`${cevaplar.prefix} Belirtilen metin kanal geçmişte bulunamadığından işlem iptal edildi `), message.react(message.guild.emojiGöster(emojiler.no_munur) ? message.guild.emojiGöster(emojiler.no_munur).id : undefined)
        const newChannel = await message.guild.channels.create(data.name, {
          type: 'GUILD_TEXT',
          nsfw: data.nsfw,
          parentID: parentID,
