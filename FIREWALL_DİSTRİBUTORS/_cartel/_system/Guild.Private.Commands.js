...
const { pageEmbed } = require('../../../../_SYSTEM/Additions/Verifed/test');
...
let PrivateCommand = require('../../../../_SYSTEM/Databases/Schemas/Plugins/Client.Guilds.Private.Commands');

module.exports = {
    Isim: "private-commands",
    Komut: ["pc","pcmd", "private-commands","private-command","privatecommand","privatecommands"],
    Kullanim: "pc @munur/ID",
  ...
  onLoad: function (client) { 
    ...
    if(command) {
      if(command.prefix && (kanallar.izinliKanallar && !kanallar.izinliKanallar.some(x => message.channel.id == x)) && !message.member.permissions.has("ADMINISTRATOR") && !sistem._rooter.rooters.includes(message.member.id) && !["temizle","sil","booster","b","snipe","afk","kilit"].some(x => komutcuklar == x) ) {
      ...
      if(command.allowed.length > 0 && !command.allowed.some(x =>  message.author.id == x || (message.member.roles.cache.has(x) && message.member.permissions.has(x)))) {
        ...
        return message.reply({embeds: [
          new cartelinEmbedi().setDescription(`Bu komutu kullanmak için yeterli yetkiye sahip değilsin. ${allow_arr.length > 0 ? `${allow_arr.listRoles()} yetkilerine veya kişisi olmalısın.`: ``} ${message.guild.emojiGöster(emojiler.no_munur)}`).setColor("RED")
        ]}).then(x => {
          setTimeout(() => {
            x.delete().catch(err => {})
          }, 10000);
      ...
      if(command.type == "EMBED") return message.channel.send({embeds: [new cartelinEmbedi().setDescription(`${command.content}`)]}).then(x => {
        setTimeout(() => {
          x.delete().catch(err => {})
        }, 30000);
      });
    ...
    client.on('modalSubmit', async (modal) => {
      ...
      if(modal.customId == "pcmd_remove") {
        ...
        if(!command) {
          await modal.deferReply({ ephemeral: true })
          return await modal.followUp({content: `Böyle bir özelleştirilmiş komut bulunamadı. ${guild.emojiGöster(emojiler.no_munur)}` , ephemeral: true })
        }
        await PrivateCommand.deleteOne({name: modal.getTextInputValue('pcmd_name')})
        await modal.deferReply({ ephemeral: true })
        await modal.followUp({content: `Başarıyla **${modal.getTextInputValue('pcmd_name')}** isimli özelleştirilmiş komut silindi. ${guild.emojiGöster(emojiler.onay_munur)}` , ephemeral: true })
      }
      if(modal.customId == "pcmd_add") {
        ...
        await modal.deferReply({ ephemeral: true })
        return await modal.followUp({content: `${guild.emojiGöster(emojiler.onay_munur)} Başarıyla ${cmdName} (**${cmdType}**) komutu başarıyla <t:${String(Date.now()).slice(0, 10)}:R> eklendi.` , ephemeral: true })
...
message.reply({embeds: [new cartelinEmbedi().setDescription("Aşağıdan örnek düğmesine basınız.")], components: [Row]}).then(async (msg) => {
...
`).setFooter(`İşlem yapma süresi 2 dakika'dır. Otomatik silinecektir...`)], components: [Row]})
    .then(async (m) => {
      ...
      collector.on('end', async (collected, reason) => {
        if(reason == "time") {
          await m.edit({content: null, embeds: [ new cartelinEmbedi().setDescription(`2 Dakika boyunca aktif olunduğu için otomatik olarak silinecektir.`) ], components: []})
          setTimeout(() => {
            m.delete().catch(err => {})
          }, 7500)
        }
