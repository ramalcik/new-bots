  ...
  onRequest: async function (client, message, args) {
    ...
    if(message.guild.channels.cache.find(x => x.name == "EX-SECURITY") && message.guild.channels.cache.find(x => x.name == "A-LOGS")) {
        ...
        .addComponents(
            ...
            await message.channel.send({components: [buttonSatır] ,embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.no_munur)} (\`#${message.guild.channels.cache.find(x => x.name == "A-LOGS").name}\` / \`#${message.guild.channels.cache.find(x => x.name == "EX-SECURITY").name}\`) kategorilere kurulmuş olarak gösterilmektedir. Aşağıdaki düğmelerden yapılmasını istediğiniz işlemi seçiniz!`)]})
            .then(x => {
...
async function kanalKur(message) {
    ...
    message.channel.send({embeds: [new cartelinEmbedi().setDescription(`${message.guild.emojiGöster(emojiler.onay_munur)} Gerekli olan tüm log ve güvenlik kanalları oluşturuldu ve otomatik olarak veri tabanına işlendi.`)]})
    message.react(message.guild.emojiGöster(emojiler.onay_munur) ? message.guild.emojiGöster(emojiler.onay_munur).id : undefined)
}